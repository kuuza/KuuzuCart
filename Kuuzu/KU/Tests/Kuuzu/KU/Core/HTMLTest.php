 ...
 */
  ...
  class HTMLTest extends \PHPUnit_Framework_TestCase {
    ...
    public function testLink() {
      $this->assertEquals('<a href="https://kuuzu.org" target="_blank">Kuuzu</a>', HTML::link('https://kuuzu.org', 'Kuuzu', 'target="_blank"'));
    }

    public function testImageWithWidthAndHeight() {
      $this->assertEquals('<img src="https://kuuzu.org/images/kuuzu.gif" border="0" alt="Kuuzu" title="Kuuzu" width="211" height="60" id="logo" />', HTML::image('https://kuuzu.org/images/kuuzu.gif', 'Kuuzu', 211, 60, 'id="logo"'));
    }

    public function testImageWithoutWidthAndHeight() {
      $this->assertEquals('<img src="https://kuuzu.org/images/kuuzu.gif" border="0" alt="Kuuzu" title="Kuuzu" id="logo" />', HTML::image('https://kuuzu.org/images/kuuzu.gif', 'Kuuzu', '', '', 'id="logo"'));
    }

    public function testIcon() {
      $this->assertEquals('<img src="public/sites/Shop/templates/kuuzu/images/icons/16x16/info.png" border="0" alt="Info" title="Info" id="iconInfo" />', HTML::icon('info.png', 'Info', '16x16', 'id="iconInfo"'));
    }

    public function testIconRaw() {
      $this->assertEquals('public/sites/Shop/templates/kuuzu/images/icons/16x16/info.png', HTML::iconRaw('info.png', '16x16'));
    }

    public function testSubmitImage() {
      $this->assertEquals('<input type="image" src="public/sites/Shop/templates/kuuzu/images/icons/16x16/edit.png" title="Edit" id="editSubmitImage" />', HTML::submitImage(HTML::iconRaw('edit.png'), 'Edit', 'id="editSubmitImage"'));
    }
    ...
    public function testButtonButton() {
      $this->assertEquals('<button id="button3" type="button" onclick="window.open(\'https://kuuzu.org\');">Kuuzu</button><script type="text/javascript">$("#button3").button({icons:{secondary:"ui-icon-tick"}}).addClass("ui-priority-secondary");</script>', HTML::button(array('href' => 'https://kuuzu.org', 'newwindow' => true, 'title' => 'Kuuzu', 'icon' => 'tick', 'iconpos' => 'right', 'priority' => 'secondary')));
    }

    public function testInputField() {
      $this->assertEquals('<input type="text" name="site" value="Kuuzu" id="ifName" />', HTML::inputField('site', 'Kuuzu', 'id="ifName"'));
    }
    ...
    public function testHiddenSessionIDField() {
      ...
      if ( $KUUZU_Session->hasStarted() && (strlen(SID) > 0) ) {
        $this->assertEquals('<input type="hidden" name="' . $KUUZU_Session->getName() . '" value="' . $KUUZU_Session->getID() . '" />', HTML::hiddenSessionIDField());
      } else {
