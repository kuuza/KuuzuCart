...
class Language
{
    ...
    {
        foreach (KUUZU::callDB('GetLanguages', null, 'Core') as $lang) {
    ...
    public function load($key, $language_code = null)
    {
        ...
        foreach (KUUZU::callDB('GetLanguageDefinitions', $def_data, 'Core') as $def) {
    ...
    public function set($code = null)
    {
        ...
        if (!isset($_COOKIE['language']) || ($_COOKIE['language'] != $this->_code)) {
            KUUZU::setCookie('language', $this->_code, time()+60*60*24*90);
        }
    ...
    public function showImage($code = null, $width = 16, $height = 10, $parameters = null)
    {
        ...
        return HTML::image(KUUZU::getPublicSiteLink('images/worldflags/' . $image_code . '.png', null, 'Shop'), $this->_languages[$code]['name'], $width, $height, $parameters);
    }
