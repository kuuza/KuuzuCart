    ...
    public static function getResponse($parameters, $driver = null) {
      if ( !isset($driver) ) {
        foreach ( static::$_drivers as $d ) {
          if ( call_user_func(array('Kuuzu\\KU\\Core\\HttpRequest\\' . $d, 'canUse')) ) {
            $driver = $d;
      ...
      if ( !isset($parameters['cafile']) && file_exists(KUUZU::BASE_DIRECTORY . 'External/cacert.pem') ) {
        $parameters['cafile'] = KUUZU::BASE_DIRECTORY . 'External/cacert.pem';
      }

      return trim(call_user_func(array('Kuuzu\\KU\\Core\\HttpRequest\\' . $driver, 'execute'), $parameters));
    }
