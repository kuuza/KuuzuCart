...
class PDOStatement extends \PDOStatement
{
    ...
    public function execute($input_parameters = [])
    {
        if (isset($this->_cache_key)) {
            ...
            if ($KUUZU_Cache->read($this->_cache_key, $this->_cache_expire)) {
                $this->_cache_data = $KUUZU_Cache->getCache();
 ...
 */
    ...
    public function setCache(string $key, int $expire = 0, bool $cache_empty = false)
    {
        ...
        if ($this->_query_call != 'prepare') {
            trigger_error('Kuuzu\\KU\\Core\\PDOStatement::setCache(): Cannot set cache (\'' . $this->_cache_key . '\') on a non-prepare query. Please change the query to a prepare() query.');
        }
    }
