...
*/
    ...
    public function startService($service) {
      if ( class_exists('Kuuzu\\KU\\Core\\Site\\Shop\\Module\\Service\\' . $service) ) {
        if ( call_user_func(array('Kuuzu\\KU\\Core\\Site\\Shop\\Module\\Service\\' . $service, 'start')) ) {
          $this->_started_services[] = $service;
        }
      } else {
        trigger_error('\'Kuuzu\\KU\\Core\\Site\\Shop\\Module\\Service\\' . $service . '\' does not exist', E_USER_ERROR);
      }
    }

    public function stopService($service) {
      if ( $this->isStarted($service) ) {
        call_user_func(array('Kuuzu\\KU\\Core\\Site\\Shop\\Module\\Service\\' . $service, 'stop'));
      }
    }
