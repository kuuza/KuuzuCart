 ...
 */
  ...
  class Customer {
    ...
    public function __construct() {
      if ( !isset($_SESSION['Kuu_Customer_data']) ) {
        $_SESSION['Kuu_Customer_data'] = $this->_data;
      }

      $this->_data =& $_SESSION['Kuu_Customer_data'];
    ...
    function setCustomerData($customer_id = -1) {
      ...
      if ( is_numeric($customer_id) && ($customer_id > 0) ) {
        $Qcustomer = $KUUZU_PDO->prepare('select customers_gender, customers_firstname, customers_lastname, customers_email_address, customers_default_address_id from :table_customers where customers_id = :customers_id');
        ...
        if ( $Qcustomer->fetch() !== false ) {
          ...
          if ( is_numeric($Qcustomer->value('customers_default_address_id')) && ($Qcustomer->value('customers_default_address_id') > 0) ) {
            $Qab = $KUUZU_PDO->prepare('select entry_country_id, entry_zone_id from :table_address_book where address_book_id = :address_book_id and customers_id = :customers_id');
      ...
      if ( sizeof($this->_data) > 0 ) {
        $_SESSION['Kuu_Customer_data'] = $this->_data;
      } elseif ( isset($_SESSION['Kuu_Customer_data']) ) {
        $this->reset();
      }
    }
    ...
    function reset() {
      ...
      if ( isset($_SESSION['Kuu_Customer_data']) ) {
        unset($_SESSION['Kuu_Customer_data']);
      }
    }
...
// HPDL integrate into class better
    public function hasProductNotifications() {
      ...
      $Qcheck = $KUUZU_PDO->prepare('select products_id from :table_products_notifications where customers_id = :customers_id limit 1');
...
// HPDL integrate into class better
    function getProductNotifications() {
      ...
      $Qproducts = $KUUZU_PDO->prepare('select pd.products_id, pd.products_name from :table_products_description pd, :table_products_notifications pn where pn.customers_id = :customers_id and pn.products_id = pd.products_id and pd.language_id = :language_id order by pd.products_name');
