 ...
 */
    ...
    public static function getListingSortLink($key, $heading) {
      ...
      return HTML::link(KUUZU::getLink(null, null, KUUZU::getAllGET(array('page', 'sort')) . '&sort=' . $key . ($direction == '+' ? '|d' : '')), $heading . (($key == $current) ? $direction : ''), 'title="' . (isset($_GET['sort']) && ($_GET['sort'] == $key) ? sprintf(KUUZU::getDef('listing_sort_ascendingly'), $heading) : sprintf(KUUZU::getDef('listing_sort_descendingly'), $heading)) . '" class="productListing-heading"');
    }
