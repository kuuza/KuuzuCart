 ...
 */
    ...
    public function __construct() {
      ...
      $Qproducts = $KUUZU_PDO->query('select min(year(products_date_added)) as min_year, max(year(products_date_added)) as max_year from :table_products limit 1');
      $Qproducts->execute();
 ...
 */
    ...
    public function execute() {
      ...
      if ( $this->hasPriceSet('from') ) {
        if ( $KUUZU_Currencies->exists($_SESSION['currency']) ) {
          $this->_price_from = $this->_price_from / $KUUZU_Currencies->value($_SESSION['currency']);
        }
      }

      if ( $this->hasPriceSet('to') ) {
        if ( $KUUZU_Currencies->exists($_SESSION['currency']) ) {
          $this->_price_to = $this->_price_to / $KUUZU_Currencies->value($_SESSION['currency']);
        }
      }
      ...
      if ( $this->hasCategory() ) {
        if ( $this->isRecursive() ) {
          ...
          $sql_query .= ' and p2c.products_id = p.products_id and p2c.products_id = pd.products_id and p2c.categories_id in (' . implode(',', $KUUZU_CategoryTree->getChildren($this->_category, $subcategories_array)) . ')';
        } else {
      ...
      $Qlisting = $KUUZU_PDO->prepare($sql_query);
      $Qlisting->bindInt(':default_flag', 1);

      if ( ($this->hasPriceSet('from') || $this->hasPriceSet('to')) && (DISPLAY_PRICE_WITH_TAX == '1') ) {
        if ( $KUUZU_Customer->isLoggedOn() ) {
          $customer_country_id = $KUUZU_Customer->getCountryID();
          $customer_zone_id = $KUUZU_Customer->getZoneID();
        } else {
      ...
      $Qlisting->bindInt(':language_id', $KUUZU_Language->getID());

      if ( $this->hasCategory() ) {
        if ( !$this->isRecursive() ) {
          $Qlisting->bindInt(':language_id_c', $KUUZU_Language->getID());
      ...
      $Qlisting->bindInt(':batch_pageset', $KUUZU_PDO->getBatchFrom((isset($_GET['page']) && is_numeric($_GET['page']) ? $_GET['page'] : 1), (int)MAX_DISPLAY_SEARCH_RESULTS));
      $Qlisting->bindInt(':batch_max_results', (int)MAX_DISPLAY_SEARCH_RESULTS);
      $Qlisting->execute();
