 ...
 */
  ...
  class Product {
    ...
    public function __construct($id) {
      ...
      if ( !empty($id) ) {
        if ( is_numeric($id) ) {
          $Qproduct = $KUUZU_PDO->prepare('select products_id as id, parent_id, products_quantity as quantity, products_price as price, products_model as model, products_tax_class_id as tax_class_id, products_types_id as type_id, products_weight as weight, products_weight_class as weight_class_id, products_date_added as date_added, manufacturers_id, has_children from :table_products where products_id = :products_id and products_status = :products_status');
          ...
          if ( $Qproduct->fetch() !== false ) {
            ...
            if ( $Qproduct->valueInt('parent_id') > 0 ) {
              $Qmaster = $KUUZU_PDO->prepare('select products_id, has_children from :table_products where products_id = :products_id and products_status = :products_status');
            ...
            if ( !empty($this->_data) ) {
              $Qdesc = $KUUZU_PDO->prepare('select products_name as name, products_description as description, products_keyword as keyword, products_tags as tags, products_url as url from :table_products_description where products_id = :products_id and language_id = :language_id');
        ...
        if ( !empty($this->_data) ) {
          $Qimages = $KUUZU_PDO->prepare('select id, image, default_flag from :table_products_images where products_id = :products_id order by sort_order');
          $Qimages->bindInt(':products_id', $this->_data['master_id']);
          $Qimages->execute();

          $this->_data['images'] = $Qimages->fetchAll();

          $Qcategory = $KUUZU_PDO->prepare('select categories_id from :table_products_to_categories where products_id = :products_id limit 1');
          ...
          if ( $this->_data['type_id'] > 0 ) {
            ...
            $Qtypes = $KUUZU_PDO->prepare('select action, module from :table_product_types_assignments where types_id = :types_id order by action, sort_order, module');
          ...
          if ( (int)$this->_data['has_children'] === 1 ) {
            ...
            $Qsubproducts = $KUUZU_PDO->prepare('select * from :table_products where parent_id = :parent_id and products_status = :products_status');
            ...
            while ( $Qsubproducts->fetch() ) {
              ...
              $Qvariants = $KUUZU_PDO->prepare('select pv.default_combo, pvg.id as group_id, pvg.title as group_title, pvg.module, pvv.id as value_id, pvv.title as value_title, pvv.sort_order as value_sort_order from :table_products_variants pv, :table_products_variants_groups pvg, :table_products_variants_values pvv where pv.products_id = :products_id and pv.products_variants_values_id = pvv.id and pvv.languages_id = :languages_id_pvv and pvv.products_variants_groups_id = pvg.id and pvg.languages_id = :languages_id_pvg order by pvg.sort_order, pvg.title');
              $Qvariants->bindInt(':products_id', $Qsubproducts->valueInt('products_id'));
              $Qvariants->bindInt(':languages_id_pvv', $KUUZU_Language->getID());
              $Qvariants->bindInt(':languages_id_pvg', $KUUZU_Language->getID());
              $Qvariants->execute();
          ...
          $Qattributes = $KUUZU_PDO->prepare('select tb.code, pa.value from :table_product_attributes pa, :table_templates_boxes tb where pa.products_id = :products_id and pa.languages_id in (0, :languages_id) and pa.id = tb.id');
          $Qattributes->bindInt(':products_id', $this->_data['master_id']);
          $Qattributes->bindInt(':languages_id', $KUUZU_Language->getID());
          $Qattributes->execute();
          ...
          if ( $KUUZU_Service->isStarted('Reviews') ) {
            $Qavg = $KUUZU_PDO->prepare('select avg(reviews_rating) as rating from :table_reviews where products_id = :products_id and languages_id = :languages_id and reviews_status = 1');
            $Qavg->bindInt(':products_id', $this->_data['master_id']);
            $Qavg->bindInt(':languages_id', $KUUZU_Language->getID());
            $Qavg->execute();
    ...
    public function getPrice($with_special = false) {
      ...
      if ( ($with_special === true) && $KUUZU_Service->isStarted('Specials') && ($new_price = Registry::get('Specials')->getPrice($this->_data['id'])) ) {
        $price = $KUUZU_Currencies->displayPriceRaw($new_price, $this->_data['tax_class_id']);
      } else {
        if ( $this->hasVariants() ) {
          $price = $KUUZU_Currencies->displayPriceRaw($this->getVariantMinPrice(), $this->_data['tax_class_id']);
        } else {
          $price = $KUUZU_Currencies->displayPriceRaw($this->_data['price'], $this->_data['tax_class_id']);
        }
      }
    ...
    public function getPriceFormated($with_special = false) {
      ...
      if ( ($with_special === true) && $KUUZU_Service->isStarted('Specials') && ($new_price = Registry::get('Specials')->getPrice($this->_data['id'])) ) {
        $price = '<s>' . $KUUZU_Currencies->displayPrice($this->_data['price'], $this->_data['tax_class_id']) . '</s> <span class="productSpecialPrice">' . $KUUZU_Currencies->displayPrice($new_price, $this->_data['tax_class_id']) . '</span>';
      } else {
        if ( $this->hasVariants() ) {
          $price = 'from&nbsp;' . $KUUZU_Currencies->displayPrice($this->getVariantMinPrice(), $this->_data['tax_class_id']);
        } else {
          $price = $KUUZU_Currencies->displayPrice($this->_data['price'], $this->_data['tax_class_id']);
        }
      }
    ...
    public function getWeight() {
      ...
      if ( $this->hasVariants() ) {
          ...
          foreach ( $variants['values'] as $group_id => $values ) {
            foreach ( $values as $value_id => $data ) {
              if ( $data['default'] === true ) {
                $weight = $KUUZU_Weight->display($variants['data']['weight'], $variants['data']['weight_class_id']);
        ...
        $weight = $KUUZU_Weight->display($this->_data['weight'], $this->_data['weight_class_id']);
      }
...
// $action mixed, string = action, array $key $value = action + single module
      ...
      if ( isset($this->_data['type_assignments'][$action_call]) ) {
        foreach ( $this->_data['type_assignments'][$action_call] as $module ) {
          ...
          if ( is_null($module_call) || ($module_call == $module) ) {
            if ( class_exists('Kuuzu\\KU\\Core\\Site\\Shop\\Module\\ProductType\\' . $module) ) {
              if ( !call_user_func(array('Kuuzu\\KU\\Core\\Site\\Shop\\Module\\ProductType\\' . $module, 'isValid'), $this) ) {
                if ( ($execute_onfail === true) && method_exists('Kuuzu\\KU\\Core\\Site\\Shop\\Module\\ProductType\\' . $module, 'onFail') ) {
                  call_user_func(array('Kuuzu\\KU\\Core\\Site\\Shop\\Module\\ProductType\\' . $module, 'onFail'), $this);
                }
    ...
    public function getManufacturer() {
      $KUUZU_Manufacturer = new Manufacturer($this->_data['manufacturers_id']);

      return $KUUZU_Manufacturer->getTitle();
    }
...
// HPDL
    ...
    public function getAttribute($code) {
      if ( !class_exists('Kuu_ProductAttributes_' . $code) ) {
      ...
      if ( class_exists('Kuu_ProductAttributes_' . $code) ) {
        return call_user_func(array('Kuu_ProductAttributes_' . $code, 'getValue'), $this->_data['attributes'][$code]);
      }
    }
    ...
    public static function checkID($id) {
      ...
      return ( (preg_match('/^[0-9]+(#?([0-9]+:?[0-9]+)+(;?([0-9]+:?[0-9]+)+)*)*$/', $id) || preg_match('/^[a-zA-Z0-9 -_]*$/', $id)) && ($id != $KUUZU_Session->getName()) );
    }
    ...
    public static function checkEntry($id) {
      ...
      $Qproduct = $KUUZU_PDO->prepare($sql_query);
    ...
    public function incrementCounter() {
      ...
      $Qupdate = $KUUZU_PDO->prepare('update :table_products_description set products_viewed = products_viewed+1 where products_id = :products_id and language_id = :language_id');
      $Qupdate->bindInt(':products_id', Products::getProductID($this->_data['id']));
      $Qupdate->bindInt(':language_id', $KUUZU_Language->getID());
      $Qupdate->execute();
    }
