 ...
 */
  ...
  class ShoppingCart {
    ...
    public function __construct() {
      if ( !isset($_SESSION['Kuu_ShoppingCart_data']) ) {
        $_SESSION['Kuu_ShoppingCart_data'] = array('contents' => array(),
                                                   'sub_total_cost' => 0,
                                                   'total_cost' => 0,
                                                   'total_weight' => 0,
                                                   'tax' => 0,
      ...
      $this->_contents =& $_SESSION['Kuu_ShoppingCart_data']['contents'];
      $this->_sub_total =& $_SESSION['Kuu_ShoppingCart_data']['sub_total_cost'];
      $this->_total =& $_SESSION['Kuu_ShoppingCart_data']['total_cost'];
      $this->_weight =& $_SESSION['Kuu_ShoppingCart_data']['total_weight'];
      $this->_tax =& $_SESSION['Kuu_ShoppingCart_data']['tax'];
      $this->_tax_groups =& $_SESSION['Kuu_ShoppingCart_data']['tax_groups'];
      $this->_shipping_boxes_weight =& $_SESSION['Kuu_ShoppingCart_data']['shipping_boxes_weight'];
      $this->_shipping_boxes =& $_SESSION['Kuu_ShoppingCart_data']['shipping_boxes'];
      $this->_shipping_address =& $_SESSION['Kuu_ShoppingCart_data']['shipping_address'];
      $this->_shipping_method =& $_SESSION['Kuu_ShoppingCart_data']['shipping_method'];
      $this->_billing_address =& $_SESSION['Kuu_ShoppingCart_data']['billing_address'];
      $this->_billing_method =& $_SESSION['Kuu_ShoppingCart_data']['billing_method'];
      $this->_shipping_quotes =& $_SESSION['Kuu_ShoppingCart_data']['shipping_quotes'];
      $this->_order_totals =& $_SESSION['Kuu_ShoppingCart_data']['order_totals'];
    }
    ...
    public function synchronizeWithDatabase() {
      ...
      foreach ( $this->_contents as $item_id => $data ) {
        ...
        if ( $db_action == 'check' ) {
          $Qproduct = $KUUZU_PDO->prepare('select item_id, quantity from :table_shopping_carts where customers_id = :customers_id and products_id = :products_id');
          $Qproduct->bindInt(':customers_id', $KUUZU_Customer->getID());
          $Qproduct->bindInt(':products_id', $data['id']);
          $Qproduct->execute();

          if ( $Qproduct->fetch() !== false ) {
            $Qupdate = $KUUZU_PDO->prepare('update :table_shopping_carts set quantity = :quantity where customers_id = :customers_id and item_id = :item_id');
        ...
        if ( $db_action == 'insert') {
          $Qid = $KUUZU_PDO->prepare('select max(item_id) as item_id from :table_shopping_carts where customers_id = :customers_id');
          $Qid->bindInt(':customers_id', $KUUZU_Customer->getID());
          $Qid->execute();

          $db_item_id = $Qid->valueInt('item_id') + 1;

          $Qnew = $KUUZU_PDO->prepare('insert into :table_shopping_carts (customers_id, item_id, products_id, quantity, date_added) values (:customers_id, :item_id, :products_id, :quantity, now())');
          $Qnew->bindInt(':customers_id', $KUUZU_Customer->getID());
          ...
          if ( isset($data['variants']) ) {
            foreach ( $data['variants'] as $variant ) {
              if ( $variant['has_custom_value'] === true ) {
                $Qnew = $KUUZU_PDO->prepare('insert into :table_shopping_carts_custom_variants_values (shopping_carts_item_id, customers_id, products_id, products_variants_values_id, products_variants_values_text) values (:shopping_carts_item_id, :customers_id, :products_id, :products_variants_values_id, :products_variants_values_text)');
...
// reset per-session cart contents, but not the database contents
      ...
      while ( $Qproducts->fetch() ) {
        if ( $Qproducts->valueInt('products_status') === 1 ) {
          ...
          if ( $Qproducts->valueInt('parent_id') > 0 ) {
            $Qcheck = $KUUZU_PDO->prepare('select products_status from :table_products where products_id = :products_id');
            $Qcheck->bindInt(':products_id', $Qproducts->valueInt('parent_id'));
            $Qcheck->execute();

            if ( $Qcheck->valueInt('products_status') === 1 ) {
              $Qvariant = $KUUZU_PDO->prepare('select pvg.id as group_id, pvg.title as group_title, pvg.module, pvv.id as value_id, pvv.title as value_title from :table_products_variants pv, :table_products_variants_values pvv, :table_products_variants_groups pvg where pv.products_id = :products_id and pv.products_variants_values_id = pvv.id and pvv.languages_id = :languages_id_pvv and pvv.products_variants_groups_id = pvg.id and pvg.languages_id = :languages_id_pvg');
              $Qvariant->bindInt(':products_id', $Qproducts->valueInt('products_id'));
              $Qvariant->bindInt(':languages_id_pvv', $KUUZU_Language->getID());
              $Qvariant->bindInt(':languages_id_pvg', $KUUZU_Language->getID());
              $Qvariant->execute();
              ...
              if ( count($variants) > 0 ) {
                foreach ( $variants as $v ) {
                  ...
                  $Qcvv = $KUUZU_PDO->prepare('select products_variants_values_text from :table_shopping_carts_custom_variants_values where customers_id = :customers_id and shopping_carts_item_id = :shopping_carts_item_id and products_id = :products_id and products_variants_values_id = :products_variants_values_id');
                  $Qcvv->bindInt(':customers_id', $KUUZU_Customer->getID());
      ...
      if ( !empty($_delete_array) ) {
        ...
        $Qdelete = $KUUZU_PDO->prepare('delete from :table_shopping_carts where customers_id = :customers_id and item_id in ("' . implode('", "', $_delete_array) . '")');
        $Qdelete->bindInt(':customers_id', $KUUZU_Customer->getID());
        $Qdelete->execute();

        $Qdelete = $KUUZU_PDO->prepare('delete from :table_shopping_carts_custom_variants_values where customers_id = :customers_id and shopping_carts_item_id in ("' . implode('", "', $_delete_array) . '")');
        $Qdelete->bindInt(':customers_id', $KUUZU_Customer->getID());
        $Qdelete->execute();
      }
    ...
    public function reset($reset_database = false) {
      ...
      if ( ($reset_database === true) && $KUUZU_Customer->isLoggedOn() ) {
        $Qdelete = $KUUZU_PDO->prepare('delete from :table_shopping_carts where customers_id = :customers_id');
        $Qdelete->bindInt(':customers_id', $KUUZU_Customer->getID());
        $Qdelete->execute();

        $Qdelete = $KUUZU_PDO->prepare('delete from :table_shopping_carts_custom_variants_values where customers_id = :customers_id');
        $Qdelete->bindInt(':customers_id', $KUUZU_Customer->getID());
        $Qdelete->execute();
      }
    ...
    public function add($product_id, $quantity = null) {
      ...
      $KUUZU_Product = new Product($product_id);

      if ( $KUUZU_Product->isValid() ) {
        if ( $this->exists($product_id) ) {
          ...
          $this->_contents[$item_id] = array('item_id' => $item_id,
                                             'id' => $product_id,
                                             'parent_id' => (int)$KUUZU_Product->getData('parent_id'),
                                             'name' => $KUUZU_Product->getTitle(),
                                             'model' => $KUUZU_Product->getModel(),
                                             'keyword' => $KUUZU_Product->getKeyword(),
                                             'image' => $KUUZU_Product->getImage(),
                                             'price' => $KUUZU_Product->getPrice(true),
                                             'quantity' => $quantity,
                                             'weight' => $KUUZU_Product->getData('weight'),
                                             'tax_class_id' => $KUUZU_Product->getData('tax_class_id'),
                                             'date_added' => DateTime::getShort(DateTime::getNow()),
                                             'weight_class_id' => $KUUZU_Product->getData('weight_class_id'));
          ...
          if ( $KUUZU_Product->getData('parent_id') > 0 ) {
            $Qvariant = $KUUZU_PDO->prepare('select pvg.id as group_id, pvg.title as group_title, pvg.module, pvv.id as value_id, pvv.title as value_title from :table_products_variants pv, :table_products_variants_values pvv, :table_products_variants_groups pvg where pv.products_id = :products_id and pv.products_variants_values_id = pvv.id and pvv.languages_id = :languages_id_pvv and pvv.products_variants_groups_id = pvg.id and pvg.languages_id = :languages_id_pvg');
            ...
            while ( $Qvariant->fetch() ) {
              ...
              if ( $KUUZU_Customer->isLoggedOn() && ($has_custom_value === true) ) {
                $Qnew = $KUUZU_PDO->prepare('insert into :table_shopping_carts_custom_variants_values (shopping_carts_item_id, customers_id, products_id, products_variants_values_id, products_variants_values_text) values (:shopping_carts_item_id, :customers_id, :products_id, :products_variants_values_id, :products_variants_values_text)');
    ...
    public function remove($item_id) {
      ...
      if ( $KUUZU_Customer->isLoggedOn() ) {
        $Qdelete = $KUUZU_PDO->prepare('delete from :table_shopping_carts where customers_id = :customers_id and item_id = :item_id');
        $Qdelete->bindInt(':customers_id', $KUUZU_Customer->getID());
        $Qdelete->bindInt(':item_id', $item_id);
        $Qdelete->execute();

        $Qdelete = $KUUZU_PDO->prepare('delete from :table_shopping_carts_custom_variants_values where customers_id = :customers_id and shopping_carts_item_id = :shopping_carts_item_id');
        $Qdelete->bindInt(':customers_id', $KUUZU_Customer->getID());
...
/* HPDL
          if (isset($data['attributes'])) {
            foreach ($data['attributes'] as $value) {
              $Qcheck = $KUUZU_PDO->prepare('select count(*) as total from :table_products_attributes pa, :table_products_attributes_download pad where pa.products_id = :products_id and pa.options_values_id = :options_values_id and pa.products_attributes_id = pad.products_attributes_id');
...
/* param $address mixed int for address book entry, or array containing address data */
    public function setShippingAddress($address) {
      ...
      if ( $KUUZU_Customer->isLoggedOn() && is_numeric($address) ) {
        $Qaddress = $KUUZU_PDO->prepare('select ab.*, z.zone_code, z.zone_name, c.countries_name, c.countries_iso_code_2, c.countries_iso_code_3, c.address_format from :table_address_book ab left join :table_zones z on (ab.entry_zone_id = z.zone_id) left join :table_countries c on (ab.entry_country_id = c.countries_id) where ab.customers_id = :customers_id and ab.address_book_id = :address_book_id');
        $Qaddress->bindInt(':customers_id', $KUUZU_Customer->getID());
    ...
    public function resetShippingAddress() {
      ...
      if ( $KUUZU_Customer->isLoggedOn() && $KUUZU_Customer->hasDefaultAddress() ) {
        $this->setShippingAddress($KUUZU_Customer->getDefaultAddressID());
      }
    }
...
/* param $address mixed int for address book entry, or array containing address data */
    ...
    public function resetBillingAddress() {
      ...
      if ( $KUUZU_Customer->isLoggedOn() && $KUUZU_Customer->hasDefaultAddress() ) {
        $this->setBillingAddress($KUUZU_Customer->getDefaultAddressID());
      }
    }
    ...
    private function _calculate($set_shipping = true) {
      $KUUZU_Weight = Registry::get('Weight');
      ...
      if ( $this->hasContents() ) {
        foreach ( $this->_contents as $data ) {
          $products_weight = $KUUZU_Weight->convert($data['weight'], $data['weight_class_id'], SHIPPING_WEIGHT_UNIT);
          $this->_weight += $products_weight * $data['quantity'];

          $tax = $KUUZU_Tax->getTaxRate($data['tax_class_id'], $this->getTaxingAddress('country_id'), $this->getTaxingAddress('zone_id'));
          $tax_description = $KUUZU_Tax->getTaxRateDescription($data['tax_class_id'], $this->getTaxingAddress('country_id'), $this->getTaxingAddress('zone_id'));

          $shown_price = $KUUZU_Currencies->addTaxRateToPrice($data['price'], $tax, $data['quantity']);
        ...
        if ( $set_shipping === true ) {
          if ( !$this->hasShippingMethod() || ($this->getShippingMethod('is_cheapest') === true) ) {
            ...
            $this->setShippingMethod($KUUZU_Shipping->getCheapestQuote(), false);
          } else {
            $KUUZU_Shipping = new Shipping($this->getShippingMethod('id'));
            $this->setShippingMethod($KUUZU_Shipping->getQuote(), false);
          }

          Registry::set('Shipping', $KUUZU_Shipping, true);
        }

        $KUUZU_OrderTotal = new OrderTotal();
        $this->_order_totals = $KUUZU_OrderTotal->getResult();

        Registry::set('OrderTotal', $KUUZU_OrderTotal, true);
      }
    }
  }
?>
