 ...
 */
    ...
    public function __construct() {
      ...
      if ( $KUUZU_Cache->read('category_tree-' . $KUUZU_Language->getCode(), 720) ) {
        $this->_data = $KUUZU_Cache->getCache();
      } else {
    ...
    protected function _buildBranch($parent_id, $level = 0) {
      ...
      if ( isset($this->_data[$parent_id]) ) {
        foreach ( $this->_data[$parent_id] as $category_id => $category ) {
          ...
          $result .= str_repeat($this->spacer_string, $this->spacer_multiplier * $level) . HTML::link(KUUZU::getLink(null, 'Index', 'cPath=' . $category_link), $link_title);
 ...
 */
    ...
    protected function _calculateProductTotals($filter_active = true) {
      ...
      if ( $filter_active === true ) {
        $Qtotals = $KUUZU_PDO->prepare($sql_query);
        $Qtotals->bindInt(':products_status', 1);
      } else {
        $Qtotals = $KUUZU_PDO->query($sql_query);
      }
