 ...
 */
    ...
    public function activateAll() {
      ...
      $Qbanner = $KUUZU_PDO->query('select banners_id, date_scheduled from :table_banners where date_scheduled != ""');
      $Qbanner->execute();
 ...
 */
    ...
    public function expireAll() {
      ...
      $Qbanner = $KUUZU_PDO->query('select b.banners_id, b.expires_date, b.expires_impressions, sum(bh.banners_shown) as banners_shown from :table_banners b, :table_banners_history bh where b.status = 1 and b.banners_id = bh.banners_id group by b.banners_id');
      $Qbanner->execute();
 ...
 */
    ...
    public function isActive($id) {
      ...
      $Qbanner = $KUUZU_PDO->prepare('select status from :table_banners where banners_id = :banners_id');
 ...
 */
    ...
    public function exists($group) {
      ...
      $Qbanner = $KUUZU_PDO->prepare($sql_query);
 ...
 */
    ...
    public function display($id = null) {
      ...
      $Qbanner = $KUUZU_PDO->prepare('select * from :table_banners where banners_id = :banners_id and status = 1');
      $Qbanner->bindInt(':banners_id', $id);
      $Qbanner->execute();
...
// HPDL create Redirect action; fix banner image location
          $banner_string = HTML::link(KUUZU::getLink('Shop', 'Index', 'Redirect&action=banner&goto=' . (int)$result['banners_id']), HTML::image('public/' . $Qbanner->value('banners_image'), $Qbanner->value('banners_title')), 'target="_blank"');
        }
 ...
 */
    ...
    public function getURL($id, $increment_click_flag = false) {
      ...
      $Qbanner = $KUUZU_PDO->prepare('select banners_url from :table_banners where banners_id = :banners_id and status = 1');
      $Qbanner->bindInt(':banners_id', $id);
      $Qbanner->execute();
 ...
 */
    ...
    private function _setStatus($id, $active_flag) {
      ...
      if ( $active_flag === true ) {
        $Qbanner = $KUUZU_PDO->prepare('update :table_banners set status = 1, date_status_change = now(), date_scheduled = NULL where banners_id = :banners_id');
      } else {
        $Qbanner = $KUUZU_PDO->prepare('update :table_banners set status = 0, date_status_change = now() where banners_id = :banners_id');
      }
 ...
 */
    ...
    private function _updateDisplayCount($id) {
      ...
      $Qcheck = $KUUZU_PDO->prepare('select count(*) as count from :table_banners_history where banners_id = :banners_id and date_format(banners_history_date, "%Y%m%d") = date_format(now(), "%Y%m%d")');
      ...
      if ( ($result !== false) && ($result['count'] > 0) ) {
        $Qbanner = $KUUZU_PDO->prepare('update :table_banners_history set banners_shown = banners_shown + 1 where banners_id = :banners_id and date_format(banners_history_date, "%Y%m%d") = date_format(now(), "%Y%m%d")');
      } else {
        $Qbanner = $KUUZU_PDO->prepare('insert into :table_banners_history (banners_id, banners_shown, banners_history_date) values (:banners_id, 1, now())');
      }
 ...
 */
    ...
    private function _updateClickCount($id) {
      ...
      if ( ($result !== false) && ($result['count'] > 0) ) {
        $Qbanner = $KUUZU_PDO->prepare('update :table_banners_history set banners_clicked = banners_clicked + 1 where banners_id = :banners_id and date_format(banners_history_date, "%Y%m%d") = date_format(now(), "%Y%m%d")');
      } else {
        $Qbanner = $KUUZU_PDO->prepare('insert into :table_banners_history (banners_id, banners_clicked, banners_history_date) values (:banners_id, 1, now())');
      }
