 ...
 */
  ...
  namespace Kuuzu\KU\Core\Site\Shop\Application\Checkout\Action\Shipping\Address;
        ...
        if ( $error === false ) {
          if ( $KUUZU_Customer->isLoggedOn() ) {
            ...
            if ( $Qab->rowCount() === 1 ) {
              ...
              $KUUZU_ShoppingCart->setShippingAddress($address_book_id);
            ...
            $KUUZU_ShoppingCart->setShippingAddress($address_array);
...
// process the selected shipping destination
        ...
        if ( AddressBook::checkEntry($_POST['ab']) ) {
          $KUUZU_ShoppingCart->setShippingAddress($_POST['ab']);

          KUUZU::redirect(KUUZU::getLink(null, null, 'Shipping', 'SSL'));
        } else {
          KUUZU::redirect(KUUZU::getLink(null, null, 'Shipping&Address', 'SSL'));
        }
      } else {
        KUUZU::redirect(KUUZU::getLink(null, null, 'Confirm', 'SSL'));
      }
