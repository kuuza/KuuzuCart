 ...
 */
  ...
  class PayPalExpressCheckout extends \Kuuzu\KU\Core\Site\Shop\PaymentModuleAbstract {
    protected function initialize() {
      ...
      $this->_title = KUUZU::getDef('paypal_express_checkout_title');
      $this->_method_title = KUUZU::getDef('paypal_express_checkout_method_title');
      ...
      if ( $this->_status === true ) {
        ...
        if ( (int)MODULE_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_ZONE > 0 ) {
          ...
          $Qcheck->bindInt(':geo_zone_id', (int)MODULE_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_ZONE);
    ...
    public function process() {
      ...
      $params = array('TOKEN' => $_SESSION['Shop']['PM']['PAYPAL']['EC']['TOKEN'],
                      ...
                      'AMT' => $KUUZU_ShoppingCart->getTotal(),
                      'CURRENCYCODE' => $KUUZU_Currencies->getCode());

      if ( $KUUZU_ShoppingCart->hasShippingAddress() ) {
        $params['SHIPTONAME'] = $KUUZU_ShoppingCart->getShippingAddress('firstname') . ' ' . $KUUZU_ShoppingCart->getShippingAddress('lastname');
        $params['SHIPTOSTREET'] = $KUUZU_ShoppingCart->getShippingAddress('street_address');
        $params['SHIPTOCITY'] = $KUUZU_ShoppingCart->getShippingAddress('city');
        $params['SHIPTOSTATE'] = $KUUZU_ShoppingCart->getShippingAddress('zone_code');
        $params['SHIPTOCOUNTRYCODE'] = $KUUZU_ShoppingCart->getShippingAddress('country_iso_code_2');
        $params['SHIPTOZIP'] = $KUUZU_ShoppingCart->getShippingAddress('postcode');
      }

      $response_array = $this->doExpressCheckoutPayment($params);

      if (($response_array['ACK'] != 'Success') && ($response_array['ACK'] != 'SuccessWithWarning')) {
        KUUZU::redirect(KUUZU::getLink(null, 'Cart', 'error_message=' . stripslashes($response_array['L_LONGMESSAGE0']), 'SSL'));
      }
    ...
    protected function setExpressCheckout($parameters) {
      ...
      $params = array('VERSION' => $this->_api_version,
                      ...
                      'RETURNURL' => KUUZU::getLink(null, null, 'ppx=retrieve', 'SSL', true, false),
                      'CANCELURL' => KUUZU::getLink(null, null, 'ppx=cancel', 'SSL', true, false));
    ...
    protected function doExpressCheckoutPayment($parameters) {
      ...
      $params = array('VERSION' => $this->_api_version,
                      ...
                      'PAYMENTACTION' => ((MODULE_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_TRANSACTION_METHOD == 'Sale') || (strlen(MODULE_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_API_USERNAME) < 1) ? 'Sale' : 'Authorization'),
                      'BUTTONSOURCE' => 'Kuuzu30_Default_EC');
    ...
    protected function initializeExpressCheckout() {
      ...
      $params = array('CURRENCYCODE' => $KUUZU_Currencies->getCode());
      ...
      foreach ( $KUUZU_ShoppingCart->getProducts() as $product ) {
        ...
        $params['L_AMT' . $line_item_no] = $KUUZU_Currencies->formatRaw($product['price']);
        ...
        $items_total += $KUUZU_Currencies->formatRaw($product['price']) * $product['quantity'];
      ...
      foreach ( $quotes_array as $quote ) {
        ...
        if ( $KUUZU_ShoppingCart->getShippingMethod('id') == $quote['id'] ) {
      ...
      if ( !is_null($cheapest_rate) ) {
        if ( (MODULE_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_INSTANT_UPDATE == '1') && ((MODULE_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_TRANSACTION_SERVER != 'Live') || ((MODULE_PAYMENT_PAYPAL_EXPRESS_CHECKOUT_TRANSACTION_SERVER == 'Live') && (ENABLE_SSL == true))) ) { // Live server requires SSL to be enabled
          $params['CALLBACK'] = KUUZU::getRPCLink (null, 'Cart', 'PayPal&ExpressCheckoutInstantUpdate', 'SSL', false, false);
...
// don't recalculate currency values as they have already been calculated
      $params['SHIPPINGAMT'] = $KUUZU_Currencies->formatRaw($KUUZU_ShoppingCart->getShippingMethod('cost'));
      $params['AMT'] = $KUUZU_Currencies->formatRaw($params['ITEMAMT'] + $params['TAXAMT'] + $params['SHIPPINGAMT'], '', 1);
      $params['MAXAMT'] = $KUUZU_Currencies->formatRaw($params['AMT'] + $expensive_rate + 100, '', 1); // safely pad higher for dynamic shipping rates (eg, USPS express)

      $response_array = $this->setExpressCheckout($params);

      if ( ($response_array['ACK'] == 'Success') || ($response_array['ACK'] == 'SuccessWithWarning') ) {
        KUUZU::redirect($paypal_url . '&token=' . $response_array['TOKEN'] . '&useraction=commit');
      }
    ...
    protected function retrieveExpressCheckout() {
      ...
      if ( ($response_array['ACK'] == 'Success') || ($response_array['ACK'] == 'SuccessWithWarning') ) {
        ...
        KUUZU::redirect(KUUZU::getLink(null, null, 'Process', 'SSL'));
      }
