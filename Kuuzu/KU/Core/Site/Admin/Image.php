 ...
 */
  ...
  class Image extends \Kuuzu\KU\Core\Site\Shop\Image {
...
// Public methods
    ...
    function resize($image, $group_id) {
      if (kuu_empty(CFG_APP_IMAGEMAGICK_CONVERT) || !file_exists(CFG_APP_IMAGEMAGICK_CONVERT)) {
    ...
    function existsInGroup($id, $group_id) {
      ...
      $Qimage = $Kuu_Database->query('select image from :table_products_images where id = :id');
    ...
    function delete($id) {
      ...
      $Qdel = $Kuu_Database->query('delete from :table_products_images where id = :id');
    ...
    function setAsDefault($id) {
      ...
      $Qimage = $Kuu_Database->query('select products_id from :table_products_images where id = :id');
      $Qimage->bindTable(':table_products_images', TABLE_PRODUCTS_IMAGES);
      $Qimage->bindInt(':id', $id);
      $Qimage->execute();

      if ($Qimage->numberOfRows() === 1) {
        $Qupdate = $Kuu_Database->query('update :table_products_images set default_flag = :default_flag where products_id = :products_id and default_flag = :default_flag');
        ...
        $Qupdate = $Kuu_Database->query('update :table_products_images set default_flag = :default_flag where id = :id');
        $Qupdate->bindTable(':table_products_images', TABLE_PRODUCTS_IMAGES);
        $Qupdate->bindInt(':default_flag', 1);
    ...
    function reorderImages($images_array) {
      ...
      foreach ($images_array as $id) {
        ...
        $Qupdate = $Kuu_Database->query('update :table_products_images set sort_order = :sort_order where id = :id');
        $Qupdate->bindTable(':table_products_images', TABLE_PRODUCTS_IMAGES);
    ...
    function show($image, $title, $parameters = '', $group = '') {
      ...
      return kuu_image('../' . DIR_WS_IMAGES . $image, $title, $width, $height, $parameters);
    }
  }
*/
?>
