 ...
 */
  ...
  class OrderTotal extends \Kuuzu\KU\Core\Site\Shop\OrderTotal {
    ...
    public function install() {
      ...
      foreach ( $KUUZU_Language->getAll() as $key => $value ) {
        if ( file_exists(KUUZU::BASE_DIRECTORY . 'Core/Site/Shop/Languages/' . $key . '/modules/order_total/' . $this->_code . '.xml') ) {
          foreach ( $KUUZU_Language->extractDefinitions($key . '/modules/order_total/' . $this->_code . '.xml') as $def ) {
            $def['id'] = $value['id'];
    ...
    public function remove() {
      ...
      if ( file_exists(KUUZU::BASE_DIRECTORY . 'Core/Site/Shop/Languages/' . $KUUZU_Language->getCode() . '/modules/order_total/' . $this->_code . '.xml') ) {
        foreach ( $KUUZU_Language->extractDefinitions($KUUZU_Language->getCode() . '/modules/order_total/' . $this->_code . '.xml') as $def ) {
