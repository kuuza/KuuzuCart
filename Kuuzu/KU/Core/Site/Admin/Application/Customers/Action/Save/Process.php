 ...
 */
  ...
  namespace Kuuzu\KU\Core\Site\Admin\Application\Customers\Action\Save;
    ...
    public static function execute(ApplicationAbstract $application) {
      ...
      if ( ACCOUNT_GENDER > 0 ) {
        if ( ($data['gender'] != 'm') && ($data['gender'] != 'f') ) {
          $KUUZU_MessageStack->add(null, KUUZU::getDef('ms_error_gender'), 'error');
          $error = true;
        }
      }

      if ( strlen(trim($data['firstname'])) < ACCOUNT_FIRST_NAME ) {
        $KUUZU_MessageStack->add(null, sprintf(KUUZU::getDef('ms_error_first_name'), ACCOUNT_FIRST_NAME), 'error');
        $error = true;
      }

      if ( strlen(trim($data['lastname'])) < ACCOUNT_LAST_NAME ) {
        $KUUZU_MessageStack->add(null, sprintf(KUUZU::getDef('ms_error_last_name'), ACCOUNT_LAST_NAME), 'error');
        $error = true;
      }

      if ( ACCOUNT_DATE_OF_BIRTH == '1' ) {
        if ( !checkdate($data['dob_month'], $data['dob_day'], $data['dob_year']) ) {
          $KUUZU_MessageStack->add(null, KUUZU::getDef('ms_error_date_of_birth'), 'error');
          $error = true;
        }
      }

      if ( strlen(trim($data['email_address'])) < ACCOUNT_EMAIL_ADDRESS ) {
        $KUUZU_MessageStack->add(null, sprintf(KUUZU::getDef('ms_error_email_address'), ACCOUNT_EMAIL_ADDRESS), 'error');
        $error = true;
      } elseif ( filter_var($data['email_address'], FILTER_VALIDATE_EMAIL) === false ) {
        $KUUZU_MessageStack->add(null, KUUZU::getDef('ms_error_email_address_invalid'), 'error');
        $error = true;
      } elseif ( Customers::emailAddressExists($data['email_address'], (isset($_GET['id']) ? $_GET['id'] : null)) ) {
        $KUUZU_MessageStack->add(null, KUUZU::getDef('ms_error_email_address_exists'), 'error');
        $error = true;
      }

      if ( ( !isset($_GET['id']) || !empty($data['password']) ) && (strlen(trim($data['password'])) < ACCOUNT_PASSWORD) ) {
        $KUUZU_MessageStack->add(null, sprintf(KUUZU::getDef('ms_error_password'), ACCOUNT_PASSWORD), 'error');
        $error = true;
      } elseif ( !empty($_POST['confirmation']) && (trim($data['password']) != trim($_POST['confirmation'])) ) {
        $KUUZU_MessageStack->add(null, KUUZU::getDef('ms_error_password_confirmation_invalid'), 'error');
        $error = true;
      }
      ...
      if ( $error === false ) {
        if ( isset($_GET['id']) && is_numeric($_GET['id']) ) {
          ...
          $customer_data = KUUZU::callDB('Admin\Customers\Get', array('email_address' => $data['email_address']));
      ...
      if ( $error === false ) {
        $KUUZU_MessageStack->add(null, KUUZU::getDef('ms_success_action_performed'), 'success');
      } else {
        $KUUZU_MessageStack->add(null, KUUZU::getDef('ms_error_action_not_performed'), 'error');
      }
