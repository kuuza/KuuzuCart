 ...
 */
  ...
  class get {
    public static function execute($id) {
      ...
      $Qproducts = $KUUZU_PDO->prepare('select p.*, pd.* from :table_products p, :table_products_description pd where p.products_id = :products_id and p.products_id = pd.products_id and pd.language_id = :language_id');
      ...
      if ( $data['has_children'] == '1' ) {
        ...
        while ( $Qsubproducts->fetch() ) {
          ...
          $Qvariants->bindInt(':languages_id', $KUUZU_Language->getID());
          $Qvariants->bindInt(':languages_id', $KUUZU_Language->getID());
          $Qvariants->execute();
      ...
      $Qattributes = $KUUZU_PDO->prepare('select id, value from :table_product_attributes where products_id = :products_id and languages_id in (0, :languages_id)');
