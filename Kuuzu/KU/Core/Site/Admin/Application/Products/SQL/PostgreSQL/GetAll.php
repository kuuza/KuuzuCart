 ...
 */
  ...
  namespace Kuuzu\KU\Core\Site\Admin\Application\Products\SQL\PostgreSQL;
 ...
 */
  ...
  class GetAll {
    public static function execute($data) {
      ...
      $Qproducts = $KUUZU_PDO->prepare($sql_query);
      $Qproducts->bindInt(':language_id', $data['language_id']);

      if ( $data['batch_pageset'] !== -1 ) {
        $Qproducts->bindInt(':batch_pageset', $KUUZU_PDO->getBatchFrom($data['batch_pageset'], $data['batch_max_results']));
      ...
      $Qtotal = $KUUZU_PDO->prepare($sql_query);
      ...
      foreach ( $result['entries'] as &$p ) {
        ...
        if ( $p['has_children'] === 1 ) {
          $Qvariants = $KUUZU_PDO->prepare('select min(products_price) as min_price, max(products_price) as max_price, sum(products_quantity) as total_quantity, min(products_status) as products_status from :table_products where parent_id = :parent_id');
