...
?>
    ...
    <div id="variantListing">
      <p style="float: right;"><?php echo HTML::button(array('type' => 'button', 'icon' => 'plus', 'title' => KUUZU::getDef('button_add'), 'params' => 'onclick="openNewVariantForm();"')); ?></p>

      <table border="0" width="100%" cellspacing="0" cellpadding="2" class="dataTable" id="productVariantsDataTable">
        <thead>
          <tr>
            <th><?php echo KUUZU::getDef('table_heading_variants'); ?></th>
...
<?php
  ...
  if ( !$new_product ) {
    $v = array('db' => $KUUZU_Application->getProductVariants($KUUZU_ObjectInfo->getInt('products_id')));
    ...
    foreach ( $v['db'] as $key => $pv ) {
      ...
      echo '          <tr id="dbVariant' . $key . '">' .
           ...
           '            <td>' . $KUUZU_Currencies->format($pv['price']) . '</td>' .
           '            <td>' . $pv['quantity'] . '</td>' .
      ...
      <table border="0" width="100%" cellspacing="0" cellpadding="2">
        <tr>
          <td width="30%" valign="top">
            <h4><?php echo KUUZU::getDef('subsection_variants'); ?></h4>

            <fieldset>
              <ul id="availableVariantsList" style="list-style-type: none; padding-left: 10px;">

<?php
  $vg = $KUUZU_Application->getVariantGroups();
...
?>
          ...
          <td width="70%" valign="top">
            ...
            <fieldset>
              <p><label for="variants_price_tax_class"><?php echo KUUZU::getDef('field_tax_class'); ?></label><?php echo HTML::selectMenu('variants_price_tax_class', $KUUZU_Application->getTaxClassesList(), null, 'onchange="updateGross(\'variants_price\');"'); ?></p>
              <p><label for="variants_price"><?php echo KUUZU::getDef('field_price_net'); ?></label><?php echo HTML::inputField('variants_price', null, 'onkeyup="updateGross(\'variants_price\')"'); ?></p>
              <p><label for="variants_price_gross"><?php echo KUUZU::getDef('field_price_gross'); ?></label><?php echo HTML::inputField('variants_price_gross', null, 'onkeyup="updateNet(\'variants_price\')"'); ?></p>
            </fieldset>

            <h4><?php echo KUUZU::getDef('subsection_data'); ?></h4>

            <fieldset>
              <p id="vstatus"><label for="variants_status"><?php echo KUUZU::getDef('field_status'); ?></label><?php echo HTML::radioField('variants_status', array(array('id' => '1', 'text' => KUUZU::getDef('status_enabled')), array('id' => '0', 'text' => KUUZU::getDef('status_disabled')))); ?></p>
              <p><label for="variants_model"><?php echo KUUZU::getDef('field_model'); ?></label><?php echo HTML::inputField('variants_model', null); ?></p>
              <p><label for="variants_quantity"><?php echo KUUZU::getDef('field_quantity'); ?></label><?php echo HTML::inputField('variants_quantity', null); ?></p>
              <p><label for="variants_weight"><?php echo KUUZU::getDef('field_weight'); ?></label><?php echo HTML::inputField('variants_weight', null, 'size="6"'). HTML::selectMenu('variants_weight_class', $KUUZU_Application->getWeightClassesList(), SHIPPING_WEIGHT_UNIT); ?></p>
              <p><label for="variants_default"><?php echo KUUZU::getDef('field_default'); ?></label><?php echo HTML::checkboxField('variants_default'); ?></p>
            </fieldset>
...
$(function() {
  $('#vstatus').buttonset();
      ...
      <p id="variantSubmitButtonsNew"><?php echo HTML::button(array('type' => 'button', 'params' => 'onclick="processVariantForm();"', 'priority' => 'primary', 'icon' => 'plus', 'title' => KUUZU::getDef('button_add'))) . ' ' . HTML::button(array('type' => 'button', 'params' => 'onclick="closeVariantForm();"', 'priority' => 'secondary', 'icon' => 'close', 'title' => KUUZU::getDef('button_cancel'))); ?></p>
      <p id="variantSubmitButtonsEdit"><?php echo HTML::button(array('type' => 'button', 'params' => 'data-vButtonType="henrysBucket"', 'priority' => 'primary', 'icon' => 'arrowrefresh-1-n', 'title' => KUUZU::getDef('button_update'))) . ' ' . HTML::button(array('type' => 'button', 'params' => 'onclick="closeVariantForm();"', 'priority' => 'secondary', 'icon' => 'close', 'title' => KUUZU::getDef('button_cancel'))); ?></p>
    </div>
  </div>
</div>

<div id="dialogDeleteVariant" title="<?php echo HTML::output(KUUZU::getDef('dialog_delete_variant_title')); ?>">
  <p><span class="ui-icon ui-icon-alert" style="float: left; margin: 0 7px 20px 0;"></span><?php echo KUUZU::getDef('dialog_delete_variant_desc'); ?></p>
</div>

<div id="dialogDeleteNewVariant" title="<?php echo HTML::output(KUUZU::getDef('dialog_delete_new_variant_title')); ?>">
  <p><span class="ui-icon ui-icon-alert" style="float: left; margin: 0 7px 20px 0;"></span><?php echo KUUZU::getDef('dialog_delete_new_variant_desc'); ?></p>
</div>

<div id="dialogDeleteDefaultVariant" title="<?php echo HTML::output(KUUZU::getDef('dialog_delete_default_variant_title')); ?>">
  <p><span class="ui-icon ui-icon-alert" style="float: left; margin: 0 7px 20px 0;"></span><?php echo KUUZU::getDef('dialog_delete_default_variant_desc'); ?></p>
</div>
...
function processVariantForm(isNew, id) {
  ...
  $.getJSON('<?php echo KUUZU::getRPCLink(null, null, 'FormatCurrency'); ?>', { value: $('#variants_price').val() }, function (response) {
