 ...
 */
  ...
  $KUUZU_ObjectInfo = new ObjectInfo(Products::get($_GET['id']));
?>
...
tinyMCE.init({
  mode : "none",
  theme : "advanced",
  language : "<?php echo substr($Kuu_Language->getCode(), 0, 2); ?>",
  height : "400",
...
<?php
  if ( is_numeric($_GET[$Kuu_Template->getModule()]) ) {
    $Kuu_ObjectInfo = new Kuu_ObjectInfo(Kuu_Products_Admin::get($_GET[$Kuu_Template->getModule()]));
    $attributes = $Kuu_ObjectInfo->get('attributes');

    $Qpd = $Kuu_Database->query('select products_name, products_description, products_keyword, products_tags, products_url, language_id from :table_products_description where products_id = :products_id');
    $Qpd->bindTable(':table_products_description', TABLE_PRODUCTS_DESCRIPTION);
    $Qpd->bindInt(':products_id', $Kuu_ObjectInfo->getInt('products_id'));
    $Qpd->execute();
  ...
  $Qtc = $Kuu_Database->query('select tax_class_id, tax_class_title from :table_tax_class order by tax_class_title');
  $Qtc->bindTable(':table_tax_class', TABLE_TAX_CLASS);
  $Qtc->execute();

  $tax_class_array = array(array('id' => '0',
                                 'text' => $Kuu_Language->get('none')));
  ...
  $Qwc = $Kuu_Database->query('select weight_class_id, weight_class_title from :table_weight_class where language_id = :language_id order by weight_class_title');
  $Qwc->bindTable(':table_weight_class', TABLE_WEIGHT_CLASS);
  $Qwc->bindInt(':language_id', $Kuu_Language->getID());
  $Qwc->execute();
...
<?php
  foreach ($tax_class_array as $tc_entry) {
    if ( $tc_entry['id'] > 0 ) {
      echo '  tax_rates["' . $tc_entry['id'] . '"] = ' . $Kuu_Tax->getTaxRate($tc_entry['id']) . ';' . "\n";
    }
  }
?>
...
<?php
  $Qvgroups = $Kuu_Database->query('select id, title, module from :table_products_variants_groups where languages_id = :languages_id order by sort_order, title');
  $Qvgroups->bindTable(':table_products_variants_groups', TABLE_PRODUCTS_VARIANTS_GROUPS);
  $Qvgroups->bindInt(':languages_id', $Kuu_Language->getID());
  $Qvgroups->execute();
  ...
  while ( $Qvgroups->next() ) {
    echo 'variants_groups[' . $Qvgroups->valueInt('id') . '] = new Array();' .
         ...
         'variants_groups[' . $Qvgroups->valueInt('id') . '][\'multiple\'] = ' . (Kuu_Variants::allowsMultipleValues($Qvgroups->value('module')) ? 'true' : 'false') . ';';
  }

  $Qvvalues = $Kuu_Database->query('select id, title from :table_products_variants_values where languages_id = :languages_id order by sort_order, title');
  $Qvvalues->bindTable(':table_products_variants_values', TABLE_PRODUCTS_VARIANTS_VALUES);
  $Qvvalues->bindInt(':languages_id', $Kuu_Language->getID());
  $Qvvalues->execute();
...
?>
  ...
  function moreFields() {
    ...
    spanFields[0].innerHTML = '<?php echo kuu_icon('attach.png') . '&nbsp;'; ?>' + variant_string;
  ...
  function setDefaultVariant(id) {
    if ( id != variants_default_combo ) {
      ...
      document.getElementById('vdc' + id).src = "<?php echo kuu_icon_raw('default.png'); ?>";

      if (variants_default_combo != null) {
        document.getElementById('vdc' + variants_default_combo).src = "<?php echo kuu_icon_raw('default_grey.png'); ?>";
      }
...
?>
  ...
  function removeImage(id) {
    $('#deleteImageDialog').dialog('option', 'buttons', {
      ...
      "Ok": function() {
        ...
        $.getJSON('<?php echo kuu_href_link_admin('rpc.php', $Kuu_Template->getModule() . '=' . $Kuu_ObjectInfo->getInt('products_id') . '&action=deleteProductImage'); ?>' + '&image=' + image[1],
          function (data) {
            getImages();
          }
        );
  ...
  function setDefaultImage(id) {
    ...
    $.getJSON('<?php echo kuu_href_link_admin('rpc.php', $Kuu_Template->getModule() . '=' . $Kuu_ObjectInfo->getInt('products_id') . '&action=setDefaultImage'); ?>' + '&image=' + image[1],
      function (data) {
        getImagesOriginals();
      }
    );
  ...
  function showImages(data) {
    for ( i=0; i<data.entries.length; i++ ) {
      ...
      var style = 'width: <?php echo $Kuu_Image->getWidth('mini') + 20; ?>px; padding: 10px; float: left; text-align: center;';

      if ( entry[1] == '1' ) { // original (products_images_groups_id)
        var onmouseover = 'this.style.backgroundColor=\'#EFEBDE\'; this.style.backgroundImage=\'url(<?php echo kuu_href_link_admin('templates/' . $Kuu_Template->getCode() . '/images/icons/16x16/drag.png'); ?>)\'; this.style.backgroundRepeat=\'no-repeat\'; this.style.backgroundPosition=\'0 0\';';
      ...
      newdiv += '<a href="' + entry[4] + '" target="_blank"><img src="<?php echo DIR_WS_HTTP_CATALOG . 'images/products/mini/'; ?>' + entry[2] + '" border="0" height="<?php echo $Kuu_Image->getHeight('mini'); ?>" alt="' + entry[2] + '" title="' + entry[2] + '" style="max-width: <?php echo $Kuu_Image->getWidth('mini') + 20; ?>px;" /></a><br />' + entry[3] + '<br />' + entry[5] + ' bytes<br />';

      if ( entry[1] == '1' ) {
        if ( entry[6] == '1' ) {
          newdiv += '<?php echo kuu_icon('default.png'); ?>&nbsp;';
        } else {
          newdiv += '<a href="#" onclick="setDefaultImage(\'image_' + entry[0] + '\');"><?php echo kuu_icon('default_grey.png'); ?></a>&nbsp;';
        }

        newdiv += '<a href="#" onclick="removeImage(\'image_' + entry[0] + '\');"><?php echo kuu_icon('trash.png'); ?></a>';
      }
    ...
    $('#imagesOriginal').sortable( {
      update: function(event, ui) {
        $.getJSON('<?php echo kuu_href_link_admin('rpc.php', $Kuu_Template->getModule() . '=' . $Kuu_ObjectInfo->getInt('products_id') . '&action=reorderImages'); ?>' + '&' + $(this).sortable('serialize'),
          function (data) {
            getImagesOthers();
          }
        );
  ...
  function getImages() {
    ...
    $.getJSON('<?php echo kuu_href_link_admin('rpc.php', $Kuu_Template->getModule() . '=' . $Kuu_ObjectInfo->getInt('products_id') . '&action=getImages'); ?>',
      function (data) {
        showImages(data);
      }
    );
  ...
  function getImagesOriginals(makeCall) {
    $('#imagesOriginal').html('<div id="showProgressOriginal" style="float: left; padding-left: 10px;"><?php echo kuu_icon('progress_ani.gif') . '&nbsp;' . $Kuu_Language->get('images_loading_from_server'); ?></div>');

    if ( makeCall != false ) {
      $.getJSON('<?php echo kuu_href_link_admin('rpc.php', $Kuu_Template->getModule() . '=' . $Kuu_ObjectInfo->getInt('products_id') . '&action=getImages&filter=originals'); ?>',
        function (data) {
          showImages(data);
        }
      );
  ...
  function getImagesOthers(makeCall) {
    $('#imagesOther').html('<div id="showProgressOther" style="float: left; padding-left: 10px;"><?php echo kuu_icon('progress_ani.gif') . '&nbsp;' . $Kuu_Language->get('images_loading_from_server'); ?></div>');

    if ( makeCall != false ) {
      $.getJSON('<?php echo kuu_href_link_admin('rpc.php', $Kuu_Template->getModule() . '=' . $Kuu_ObjectInfo->getInt('products_id') . '&action=getImages&filter=others'); ?>',
        function (data) {
          showImages(data);
        }
      );
  ...
  function assignLocalImages() {
    ...
    $.getJSON('<?php echo kuu_href_link_admin('rpc.php', $Kuu_Template->getModule() . '=' . $Kuu_ObjectInfo->getInt('products_id') . '&action=assignLocalImages'); ?>' + '&' + selectedFiles,
      function (data) {
...
?>
  ...
  function getLocalImages() {
    ...
    $.getJSON('<?php echo kuu_href_link_admin('rpc.php', $Kuu_Template->getModule() . '&action=getLocalImages'); ?>',
      function (data) {
        var i = 0;
...
<div id="deleteImageDialog" title="<?php echo $Kuu_Language->get('action_heading_delete_image'); ?>"><p><?php echo $Kuu_Language->get('introduction_delete_image'); ?></p></div>
...
<h1><?php echo (isset($Kuu_ObjectInfo) && isset($products_name[$Kuu_Language->getID()])) ? $products_name[$Kuu_Language->getID()] : $Kuu_Language->get('heading_title_new_product'); ?></h1>
...
<form name="product" class="dataForm" action="<?php echo kuu_href_link_admin(FILENAME_DEFAULT, $Kuu_Template->getModule() . '=' . (isset($Kuu_ObjectInfo) ? $Kuu_ObjectInfo->getInt('products_id') : '') . '&cID=' . $_GET['cID'] . '&action=save'); ?>" method="post" enctype="multipart/form-data">

<div id="mainTabs">
  <ul>
    <li><?php echo kuu_link_object('#section_general_content', $Kuu_Language->get('section_general')); ?></li>
    <li><?php echo kuu_link_object('#section_data_content', $Kuu_Language->get('section_data')); ?></li>
    <li><?php echo kuu_link_object('#section_images_content', $Kuu_Language->get('section_images')); ?></li>
    <li><?php echo kuu_link_object('#section_variants_content', $Kuu_Language->get('section_variants')); ?></li>
    <li><?php echo kuu_link_object('#section_categories_content', $Kuu_Language->get('section_categories')); ?></li>
  </ul>
...
<?php
  foreach ( $Kuu_Language->getAll() as $l ) {
    echo '<li>' . kuu_link_object('#languageTabs_' . $l['code'], $Kuu_Language->showImage($l['code']) . '&nbsp;' . $l['name']) . '</li>';
  }
?>
...
?>
      ...
      <div id="languageTabs_<?php echo $l['code']; ?>">
        <fieldset>
          <div><label for="<?php echo 'products_name[' . $l['id'] . ']'; ?>"><?php echo $Kuu_Language->get('field_name'); ?></label><?php echo kuu_draw_input_field('products_name[' . $l['id'] . ']', (isset($Kuu_ObjectInfo) && isset($products_name[$l['id']]) ? $products_name[$l['id']] : null)); ?></div>
          <div><label for="<?php echo 'products_description[' . $l['id'] . ']'; ?>"><?php echo $Kuu_Language->get('field_description'); ?></label><?php echo kuu_draw_textarea_field('products_description[' . $l['id'] . ']', (isset($Kuu_ObjectInfo) && isset($products_description[$l['id']]) ? $products_description[$l['id']] : null)); ?><div style="width: 58.5%; text-align: right;"><?php echo '<a href="javascript:toggleEditor(\'products_description[' . $l['id'] . ']\');">' . $Kuu_Language->get('toggle_html_editor') . '</a>'; ?></div></div>
          <div><label for="<?php echo 'products_keyword[' . $l['id'] . ']'; ?>"><?php echo $Kuu_Language->get('field_keyword'); ?></label><?php echo kuu_draw_input_field('products_keyword[' . $l['id'] . ']', (isset($Kuu_ObjectInfo) && isset($products_keyword[$l['id']]) ? $products_keyword[$l['id']] : null)); ?></div>
          <div><label for="<?php echo 'products_tags[' . $l['id'] . ']'; ?>"><?php echo $Kuu_Language->get('field_tags'); ?></label><?php echo kuu_draw_input_field('products_tags[' . $l['id'] . ']', (isset($Kuu_ObjectInfo) && isset($products_tags[$l['id']]) ? $products_tags[$l['id']] : null)); ?></div>
          <div><label for="<?php echo 'products_url[' . $l['id'] . ']'; ?>"><?php echo $Kuu_Language->get('field_url'); ?></label><?php echo kuu_draw_input_field('products_url[' . $l['id'] . ']', (isset($Kuu_ObjectInfo) && isset($products_url[$l['id']]) ? $products_url[$l['id']] : null)); ?></div>
        </fieldset>
      </div>
...
<?php
  $data_width = ( isset($Kuu_ObjectInfo) && ((int)$Kuu_ObjectInfo->get('has_children') === 1) ) ? '100%' : '50%';

  if ( !isset($Kuu_ObjectInfo) || (isset($Kuu_ObjectInfo) && ($Kuu_ObjectInfo->getInt('has_children') !== 1)) ) {
?>

        <td width="<?php echo $data_width;?>" height="100%" valign="top">
          <fieldset style="height: 100%;">
            <legend><?php echo $Kuu_Language->get('subsection_price'); ?></legend>

            <div><label for="tax_class0"><?php echo $Kuu_Language->get('field_tax_class'); ?></label><?php echo kuu_draw_pull_down_menu('products_tax_class_id', $tax_class_array, (isset($Kuu_ObjectInfo) ? $Kuu_ObjectInfo->getInt('products_tax_class_id') : null), 'id="tax_class0" onchange="updateGross(\'products_price0\');"'); ?></div>
            <div><label for="products_price0"><?php echo $Kuu_Language->get('field_price_net'); ?></label><?php echo kuu_draw_input_field('products_price', (isset($Kuu_ObjectInfo) ? $Kuu_ObjectInfo->get('products_price') : null), 'id="products_price0" onkeyup="updateGross(\'products_price0\')"'); ?></div>
            <div><label for="products_price0_gross"><?php echo $Kuu_Language->get('field_price_gross'); ?></label><?php echo kuu_draw_input_field('products_price_gross', (isset($Kuu_ObjectInfo) ? $Kuu_ObjectInfo->get('products_price') : null), 'id="products_price0_gross" onkeyup="updateNet(\'products_price0\')"'); ?></div>
          </fieldset>
...
?>
        ...
        <td width="<?php echo $data_width;?>" height="100%" valign="top">
          <fieldset style="height: 100%;">
            <legend><?php echo $Kuu_Language->get('subsection_data'); ?></legend>

            <div><label for="products_status"><?php echo $Kuu_Language->get('field_status'); ?></label><?php echo kuu_draw_radio_field('products_status', array(array('id' => '1', 'text' => $Kuu_Language->get('status_enabled')), array('id' => '0', 'text' => $Kuu_Language->get('status_disabled'))), (isset($Kuu_ObjectInfo) ? $Kuu_ObjectInfo->get('products_status') : '0')); ?></div>

<?php
  if ( isset($Kuu_ObjectInfo) && ($Kuu_ObjectInfo->getInt('has_children') !== 1) ) {
?>

            <div><label for="products_model"><?php echo $Kuu_Language->get('field_model'); ?></label><?php echo kuu_draw_input_field('products_model', (isset($Kuu_ObjectInfo) ? $Kuu_ObjectInfo->get('products_model') : null)); ?></div>
            <div><label for="products_quantity"><?php echo $Kuu_Language->get('field_quantity'); ?></label><?php echo kuu_draw_input_field('products_quantity', (isset($Kuu_ObjectInfo) ? $Kuu_ObjectInfo->get('products_quantity') : null)); ?></div>
            <div><label for="products_weight"><?php echo $Kuu_Language->get('field_weight'); ?></label><?php echo kuu_draw_input_field('products_weight', (isset($Kuu_ObjectInfo) ? $Kuu_ObjectInfo->get('products_weight') : null)) . kuu_draw_pull_down_menu('products_weight_class', $weight_class_array, (isset($Kuu_ObjectInfo) ? $Kuu_ObjectInfo->get('products_weight_class') : SHIPPING_WEIGHT_UNIT)); ?></div>
...
<?php
  if ( isset($Kuu_ObjectInfo) && ($Kuu_ObjectInfo->getInt('has_children') === 1) ) {
    echo kuu_draw_hidden_field('products_tax_class_id', 0) . kuu_draw_hidden_field('products_price', 0) . kuu_draw_hidden_field('products_model') . kuu_draw_hidden_field('products_quantity', 0), kuu_draw_hidden_field('products_weight', 0), kuu_draw_hidden_field('products_weight_class', 0);
  }
?>
...
<?php
  $Qattributes = $Kuu_Database->query('select id, code from :table_templates_boxes where modules_group = :modules_group order by code');
  ...
  while ( $Qattributes->next() ) {
    ...
    if ( !class_exists('Kuu_ProductAttributes_' . $module) ) {
    ...
    if ( class_exists('Kuu_ProductAttributes_' . $module) ) {
      $module = 'Kuu_ProductAttributes_' . $module;
      $module = new $module();
?>
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td width="100%" height="100%" valign="top">
          <fieldset style="height: 100%;">
            <legend><?php echo $Kuu_Language->get('subsection_new_image'); ?></legend>

            <div style="float: right;">
              <a href="#" id="remoteFilesLink" onclick="switchImageFilesView('remote');" style="background-color: #E5EFE5;"><?php echo $Kuu_Language->get('image_remote_upload'); ?></a> | <a href="#" id="localFilesLink" onclick="switchImageFilesView('local');"><?php echo $Kuu_Language->get('image_local_files'); ?></a>
            </div>
...
<?php
    if ( isset($Kuu_ObjectInfo) ) {
      echo '<input type="button" id="uploadFile" value="' . $Kuu_Language->get('button_send_to_server') . '" class="operationButton" /><div id="showProgress" style="display: none; padding-left: 10px;">' . kuu_icon('progress_ani.gif') . '&nbsp;' . $Kuu_Language->get('image_upload_progress') . '</div>';
    } else {
      echo kuu_draw_file_field('products_image');
    }
?>
            </div>
...
<script type="text/javascript"><!--
  $('#uploadFile').upload( {
    ...
    action: '<?php echo kuu_href_link_admin('rpc.php', $Kuu_Template->getModule() . '=' . $Kuu_ObjectInfo->getInt('products_id') . '&action=fileUpload'); ?>',
    onSubmit: function() {
...
?>
            ...
            <div id="localFiles" style="display: none;">
              <p><?php echo $Kuu_Language->get('introduction_select_local_images'); ?></p>

              <select id="localImagesSelection" name="localimages[]" size="5" multiple="multiple" style="width: 100%;"></select>

              <div id="showProgressGetLocalImages" style="display: none; float: right; padding-right: 10px;"><?php echo kuu_icon('progress_ani.gif') . '&nbsp;' . $Kuu_Language->get('image_retrieving_local_files'); ?></div>

              <p><?php echo realpath('../images/products/_upload'); ?></p>

<?php
    if ( isset($Kuu_ObjectInfo) ) {
      echo '<input type="button" value="Assign To Product" class="operationButton" onclick="assignLocalImages();" /><div id="showProgressAssigningLocalImages" style="display: none; padding-left: 10px;">' . kuu_icon('progress_ani.gif') . '&nbsp;' . $Kuu_Language->get('image_multiple_upload_progress') . '</div>';
    }
?>
...
?>
          ...
          <fieldset style="height: 100%;">
            <legend><?php echo $Kuu_Language->get('subsection_original_images'); ?></legend>

            <div id="imagesOriginal" style="overflow: auto;"></div>
          </fieldset>

          <fieldset style="height: 100%;">
            <legend><?php echo $Kuu_Language->get('subsection_images'); ?></legend>
...
<?php
  ...
  while ($Qvgroups->next()) {
    ...
    if ( Kuu_Variants::allowsMultipleValues($Qvgroups->value('module')) ) {
    ...
    $Qvvalues = $Kuu_Database->query('select id, title from :table_products_variants_values where products_variants_groups_id = :products_variants_groups_id and languages_id = :languages_id order by sort_order, title');
    $Qvvalues->bindTable(':table_products_variants_values', TABLE_PRODUCTS_VARIANTS_VALUES);
...
?>
        ...
        <td width="65%" valign="top">
          <fieldset>
            <legend><?php echo $Kuu_Language->get('subsection_assigned_variants'); ?></legend>
...
<?php
  ...
  if ( isset($Kuu_ObjectInfo) ) {
    $Qvariants = $Kuu_Database->query('select * from :table_products where parent_id = :parent_id');
    $Qvariants->bindTable(':table_products', TABLE_PRODUCTS);
    $Qvariants->bindInt(':parent_id', $Kuu_ObjectInfo->getInt('products_id'));
    $Qvariants->execute();
    ...
    while ( $Qvariants->next() ) {
      ...
      $Qcombos->bindInt(':languages_id', $Kuu_Language->getID());
      $Qcombos->bindInt(':languages_id', $Kuu_Language->getID());
      $Qcombos->execute();
...
?>
            ...
            <div id="variant<?php echo $counter; ?>" class="attributeAdd" onclick="activateVariant(this);">
              <table border="0" width="100%" cellspacing="0" cellpadding="2">
                <tr>
                  <td colspan="2"><div style="float: right;"><?php echo '<a href="javascript:setDefaultVariant(\'' . $counter . '\');">' . kuu_icon((($variants_default_combo === $counter) ? 'default.png' : 'default_grey.png'), null, null, 'id="vdc' . $counter . '"') . '</a>'; ?>&nbsp;<a href="javascript:removeVariant('variant<?php echo $counter; ?>');"><?php echo kuu_icon('trash.png'); ?></a></div><span style="font-weight: bold;"><?php echo kuu_icon('attach.png') . '&nbsp;' . $variants_string; ?></span></td>
                </tr>
                <tr>
                  <td width="50%" height="100%" valign="top">
                    <fieldset style="height: 100%;">
                      ...
                      <table border="0" width="100%" cellspacing="0" cellpadding="2">
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_tax_class'); ?></td>
                          <td><?php echo kuu_draw_pull_down_menu('variants_tax_class_id[' . $counter . ']', $tax_class_array, $Qvariants->valueInt('products_tax_class_id'), 'id="tax_class' . $counter . '" onchange="updateGross(\'variants_price' . $counter . '\');"'); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_price_net'); ?></td>
                          <td><?php echo kuu_draw_input_field('variants_price[' . $counter . ']', $Qvariants->value('products_price'), 'id="variants_price' . $counter . '" onkeyup="updateGross(\'variants_price' . $counter . '\')"'); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_price_gross'); ?></td>
                          <td><?php echo kuu_draw_input_field('variants_price_gross[' . $counter . ']', $Qvariants->value('products_price'), 'id="variants_price' . $counter . '_gross" onkeyup="updateNet(\'variants_price' . $counter . '\')"'); ?></td>
                        </tr>
                      </table>
                  ...
                  <td width="50%" height="100%" valign="top">
                    <fieldset style="height: 100%;">
                      ...
                      <table border="0" width="100%" cellspacing="0" cellpadding="2">
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_model'); ?></td>
                          <td><?php echo kuu_draw_input_field('variants_model[' . $counter . ']', $Qvariants->value('products_model')); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_quantity'); ?></td>
                          <td><?php echo kuu_draw_input_field('variants_quantity[' . $counter . ']', $Qvariants->value('products_quantity')) . kuu_draw_hidden_field('variants_combo[' . $counter . ']', $variants_combo_string, 'id="variants_combo_' . $counter . '"') . kuu_draw_hidden_field('variants_combo_db[' . $counter . ']', $Qvariants->valueInt('products_id')); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_weight'); ?></td>
                          <td><?php echo kuu_draw_input_field('variants_weight[' . $counter . ']', $Qvariants->value('products_weight'), 'size="6"'). '&nbsp;' . kuu_draw_pull_down_menu('variants_weight_class[' . $counter . ']', $weight_class_array, $Qvariants->value('products_weight_class')); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_status'); ?></td>
                          <td><?php echo kuu_draw_radio_field('variants_status[' . $counter . ']', array(array('id' => '1', 'text' => $Kuu_Language->get('status_enabled')), array('id' => '0', 'text' => $Kuu_Language->get('status_disabled'))), $Qvariants->value('products_status')); ?></td>
                        </tr>
                      </table>
                    </fieldset>
                  </td>
...
<?php
  echo kuu_draw_hidden_field('variants_default_combo', $variants_default_combo, 'id="variants_default_combo"');
...
?>
            ...
            <div id="readroot" style="display: none" class="attributeAdd" onclick="activateVariant(this);">
              <table border="0" width="100%" cellspacing="0" cellpadding="2">
                <tr>
                  <td colspan="2"><div style="float: right;"><a href="#" name="default"><?php echo kuu_icon('default_grey.png', null, null, 'name="vdcnew"'); ?></a>&nbsp;<a href="#" name="trash"><?php echo kuu_icon('trash.png'); ?></a></div><span style="font-weight: bold;"><?php echo kuu_icon('attach.png') . '&nbsp;'; ?></span></td>
                </tr>
                <tr>
                  <td width="50%" height="100%" valign="top">
                    <fieldset style="height: 100%;">
                      ...
                      <table border="0" width="100%" cellspacing="0" cellpadding="2">
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_tax_class'); ?></td>
                          <td><?php echo kuu_draw_pull_down_menu('new_variants_tax_class_id', $tax_class_array, null, 'disabled="disabled"'); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_price_net'); ?></td>
                          <td><?php echo kuu_draw_input_field('new_variants_price', null, 'disabled="disabled"'); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_price_gross'); ?></td>
                          <td><?php echo kuu_draw_input_field('new_variants_price_gross', null, 'disabled="disabled"'); ?></td>
                        </tr>
                      </table>
                    </fieldset>
                  </td>
                  <td width="50%" height="100%" valign="top">
                    <fieldset style="height: 100%;">
                      ...
                      <table border="0" width="100%" cellspacing="0" cellpadding="2">
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_model'); ?></td>
                          <td><?php echo kuu_draw_input_field('new_variants_model', null, 'disabled="disabled"'); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_quantity'); ?></td>
                          <td><?php echo kuu_draw_input_field('new_variants_quantity', null, 'disabled="disabled"') . kuu_draw_hidden_field('new_variants_combo', null, 'disabled="disabled"'); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_weight'); ?></td>
                          <td><?php echo kuu_draw_input_field('new_variants_weight', null, 'size="6" disabled="disabled"'). '&nbsp;' . kuu_draw_pull_down_menu('new_variants_weight_class', $weight_class_array, SHIPPING_WEIGHT_UNIT, 'disabled="disabled"'); ?></td>
                        </tr>
                        <tr>
                          <td><?php echo $Kuu_Language->get('field_status'); ?></td>
                          <td><?php echo kuu_draw_radio_field('new_variants_status', array(array('id' => '1', 'text' => $Kuu_Language->get('status_enabled')), array('id' => '0', 'text' => $Kuu_Language->get('status_disabled'))), '0', 'disabled="disabled"'); ?></td>
                        </tr>
                      </table>
                    </fieldset>
                  </td>
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2" class="dataTable">
          <thead>
            <tr>
              <th width="20">&nbsp;</th>
              <th><?php echo $Kuu_Language->get('table_heading_categories'); ?></th>
            </tr>
          </thead>
          <tbody>
<?php
  ...
  $assignedCategoryTree = new Kuu_CategoryTree();
  ...
  foreach ($assignedCategoryTree->getArray() as $value) {
    echo '          <tr onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);">' . "\n" .
         '            <td>' . kuu_draw_checkbox_field('categories[]', $value['id'], in_array($value['id'], $product_categories_array), 'id="categories_' . $value['id'] . '"') . '</td>' . "\n" .
