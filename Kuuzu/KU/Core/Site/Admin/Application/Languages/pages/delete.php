...
<div class="infoBox">
  <h3><?php echo HTML::icon('trash.png') . ' ' . $KUUZU_ObjectInfo->getProtected('name'); ?></h3>

<?php
  if ( $KUUZU_ObjectInfo->get('code') == DEFAULT_LANGUAGE ) {
?>

  <p><?php echo '<b>' . KUUZU::getDef('introduction_delete_language_invalid') . '</b>'; ?></p>
...
?>
  ...
  <form name="lDelete" class="dataForm" action="<?php echo KUUZU::getLink(null, null, 'Delete&Process&id=' . $_GET['id']); ?>" method="post">

  <p><?php echo KUUZU::getDef('introduction_delete_language'); ?></p>

  <p><?php echo '<b>' . $KUUZU_ObjectInfo->getProtected('name') . '</b>'; ?></p>
