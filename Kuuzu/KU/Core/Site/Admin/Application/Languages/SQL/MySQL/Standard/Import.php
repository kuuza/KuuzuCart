  ...
  class Import {
    public static function execute($data) {
      ...
      if ( $Qlanguage->isError() ) {
        ...
        if ( !isset($data['id']) ) {
          $data['id'] = $KUUZU_PDO->lastInsertId();
        }

        if ( $data['import_type'] == 'replace' ) {
          $Qdel =  $KUUZU_PDO->prepare('delete from :table_languages_definitions where languages_id = :languages_id');
      ...
      if ( $error === false ) {
        foreach ( $data['definitions'] as $def ) {
          ...
          if ( $data['import_type'] == 'replace' ) {
            ...
            $Qcheck = $KUUZU_PDO->prepare('select definition_key, content_group from :table_languages_definitions where definition_key = :definition_key and languages_id = :languages_id and content_group = :content_group');
          ...
          if ( ($insert === true) || ($update === true) ) {
            if ( $insert === true ) {
              ...
              $Qdef = $KUUZU_PDO->prepare('update :table_languages_definitions set content_group = :content_group, definition_key = :definition_key, definition_value = :definition_value where definition_key = :definition_key and languages_id = :languages_id and content_group = :content_group');
      ...
      if ( $add_category_and_product_placeholders === true ) {
        if ( $error === false ) {
          $Qcategories = $KUUZU_PDO->prepare('select categories_id, categories_name from :table_categories_description where language_id = :language_id');
          $Qcategories->bindInt(':language_id', $data['default_language_id']);
          $Qcategories->execute();

          while ( $Qcategories->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_categories_description (categories_id, language_id, categories_name) values (:categories_id, :language_id, :categories_name)');
        ...
        if ( $error === false ) {
          $Qproducts = $KUUZU_PDO->prepare('select products_id, products_name, products_description, products_keyword, products_tags, products_url from :table_products_description where language_id = :language_id');
          $Qproducts->bindInt(':language_id', $data['default_language_id']);
          $Qproducts->execute();

          while ( $Qproducts->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_products_description (products_id, language_id, products_name, products_description, products_keyword, products_tags, products_url) values (:products_id, :language_id, :products_name, :products_description, :products_keyword, :products_tags, :products_url)');
        ...
        if ( $error === false ) {
          $Qattributes = $KUUZU_PDO->prepare('select id, products_id, value from :table_product_attributes where languages_id = :languages_id');
          $Qattributes->bindInt(':languages_id', $data['default_language_id']);
          $Qattributes->execute();

          while ( $Qattributes->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_product_attributes (id, products_id, languages_id, value) values (:id, :products_id, :languages_id, :value)');
        ...
        if ( $error === false ) {
          $Qgroups = $KUUZU_PDO->prepare('select id, title, sort_order, module from :table_products_variants_groups where languages_id = :languages_id');
          $Qgroups->bindInt(':languages_id', $data['default_language_id']);
          $Qgroups->execute();

          while ( $Qgroups->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_products_variants_groups (id, languages_id, title, sort_order, module) values (:id, :languages_id, :title, :sort_order, :module)');
        ...
        if ( $error === false ) {
          $Qvalues = $KUUZU_PDO->prepare('select id, products_variants_groups_id, title, sort_order from :table_products_variants_values where languages_id = :languages_id');
          $Qvalues->bindInt(':languages_id', $data['default_language_id']);
          $Qvalues->execute();

          while ( $Qvalues->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_products_variants_values (id, languages_id, products_variants_groups_id, title, sort_order) values (:id, :languages_id, :products_variants_groups_id, :title, :sort_order)');
        ...
        if ( $error === false ) {
          $Qmanufacturers = $KUUZU_PDO->prepare('select manufacturers_id, manufacturers_url from :table_manufacturers_info where languages_id = :languages_id');
          $Qmanufacturers->bindInt(':languages_id', $data['default_language_id']);
          $Qmanufacturers->execute();

          while ( $Qmanufacturers->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_manufacturers_info (manufacturers_id, languages_id, manufacturers_url) values (:manufacturers_id, :languages_id, :manufacturers_url)');
        ...
        if ( $error === false ) {
          $Qstatus = $KUUZU_PDO->prepare('select orders_status_id, orders_status_name from :table_orders_status where language_id = :language_id');
          $Qstatus->bindInt(':language_id', $data['default_language_id']);
          $Qstatus->execute();

          while ( $Qstatus->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_orders_status (orders_status_id, language_id, orders_status_name) values (:orders_status_id, :language_id, :orders_status_name)');
        ...
        if ( $error === false ) {
          $Qstatus = $KUUZU_PDO->prepare('select id, status_name from :table_orders_transactions_status where language_id = :language_id');
          $Qstatus->bindInt(':language_id', $data['default_language_id']);
          $Qstatus->execute();

          while ( $Qstatus->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_orders_transactions_status (id, language_id, status_name) values (:id, :language_id, :status_name)');
        ...
        if ( $error === false ) {
          $Qstatus = $KUUZU_PDO->prepare('select id, title, css_key from :table_shipping_availability where languages_id = :languages_id');
          $Qstatus->bindInt(':languages_id', $data['default_language_id']);
          $Qstatus->execute();

          while ( $Qstatus->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_shipping_availability (id, languages_id, title, css_key) values (:id, :languages_id, :title, :css_key)');
            $Qinsert->bindInt(':id', $Qstatus->valueInt('id'));
            $Qinsert->bindInt(':languages_id', $data['id']);
        ...
        if ( $error === false ) {
          $Qstatus = $KUUZU_PDO->prepare('select weight_class_id, weight_class_key, weight_class_title from :table_weight_classes where language_id = :language_id');
          $Qstatus->bindInt(':language_id', $data['default_language_id']);
          $Qstatus->execute();

          while ( $Qstatus->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_weight_classes (weight_class_id, weight_class_key, language_id, weight_class_title) values (:weight_class_id, :weight_class_key, :language_id, :weight_class_title)');
        ...
        if ( $error === false ) {
          $Qgroup = $KUUZU_PDO->prepare('select id, title, code, size_width, size_height, force_size from :table_products_images_groups where language_id = :language_id');
          $Qgroup->bindInt(':language_id', $data['default_language_id']);
          $Qgroup->execute();

          while ( $Qgroup->fetch() ) {
            $Qinsert = $KUUZU_PDO->prepare('insert into :table_products_images_groups (id, language_id, title, code, size_width, size_height, force_size) values (:id, :language_id, :title, :code, :size_width, :size_height, :force_size)');
