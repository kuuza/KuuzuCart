 ...
 */
    ...
    protected $_page_tags = array('generator' => array('Kuuzu Cart'));
 ...
 */
    ...
    function getBoxModules($group) {
      if ( !isset($this->Kuu_Modules_Boxes) ) {
        $this->Kuu_Modules_Boxes = new Modules('Box');
      }

      return $this->Kuu_Modules_Boxes->getGroup($group);
    }
 ...
 */
    ...
    function getContentModules($group) {
      if ( !isset($this->Kuu_Modules_Content) ) {
        $this->Kuu_Modules_Content = new Modules('Content');
      }

      return $this->Kuu_Modules_Content->getGroup($group);
    }
    ...
    public function getTemplateFile($file = null) {
      ...
      $file_location = KUUZU::BASE_DIRECTORY . 'Core/Site/' . KUUZU::getSite() . '/templates/' . $file;

      if ( file_exists(KUUZU::BASE_DIRECTORY . 'Custom/Site/' . KUUZU::getSite() . '/templates/' . $file) ) {
        $file_location = KUUZU::BASE_DIRECTORY . 'Custom/Site/' . KUUZU::getSite() . '/templates/' . $file;
      }
 ...
 */
    ...
    public function getPageContentsFile() {
      $file_location = KUUZU::BASE_DIRECTORY . 'Core/Site/' . KUUZU::getSite() . '/Application/' . KUUZU::getSiteApplication() . '/pages/' . $this->getPageContentsFilename();

      if ( file_exists(KUUZU::BASE_DIRECTORY . 'Custom/Site/' . KUUZU::getSite() . '/Application/' . KUUZU::getSiteApplication() . '/pages/' . $this->getPageContentsFilename()) ) {
        $file_location = KUUZU::BASE_DIRECTORY . 'Custom/Site/' . KUUZU::getSite() . '/Application/' . KUUZU::getSiteApplication() . '/pages/' . $this->getPageContentsFilename();
      }
 ...
 */
    ...
    public static function getTemplates() {
      return KUUZU::callDB('GetTemplates', null, 'Core');
    }
 ...
 */
    ...
    public function parseContent($content, $whitelist = null) {
      ...
      $le = function ($matches) use (&$le, &$pattern, $whitelist) {
        ...
        if ( class_exists('Kuuzu\\KU\\Core\\Site\\' . KUUZU::getSite() . '\\Module\\Template\\Tag\\' . $matches[1]) ) {
          $tag_class = 'Kuuzu\\KU\\Core\\Site\\' . KUUZU::getSite() . '\\Module\\Template\\Tag\\' . $matches[1];
        } elseif ( class_exists('Kuuzu\\KU\\Core\\Template\\Tag\\' . $matches[1]) ) {
          $tag_class = 'Kuuzu\\KU\\Core\\Template\\Tag\\' . $matches[1];
        }

        if ( isset($tag_class) ) {
          if ( (isset($loaded_tags[$matches[1]]) && ($loaded_tags[$matches[1]] === true) ) || is_subclass_of($tag_class, 'Kuuzu\\KU\\Core\\Template\\TagAbstract') ) {
 ...
 */
    ...
    public function getValue($key) {
      if ( !$this->valueExists($key) ) {
        trigger_error('KUUZU_Template::getValue - ' . $key . ' is not set');
 ...
 */
    ...
    public function setValue($key, $value, $force = false) {
      if ( $this->valueExists($key, false) && ($force !== true) ) {
        trigger_error('KUUZU_Template::setValue - ' . $key . ' already set and is not forced to be replaced');
 ...
 */
    ...
    public function getBaseUrl() {
      if ( !isset($this->_base_url) ) {
        if ( (KUUZU::getRequestType() == 'SSL') && (KUUZU::getConfig('enable_ssl') == 'true') ) {
          $this->_base_url = KUUZU::getConfig('https_server') . KUUZU::getConfig('dir_ws_https_server');
        } else {
          $this->_base_url = KUUZU::getConfig('http_server') . KUUZU::getConfig('dir_ws_http_server');
        }
      }
