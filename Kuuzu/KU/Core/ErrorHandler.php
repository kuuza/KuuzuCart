  ...
  class ErrorHandler {
    ...
    public static function initialize() {
      ...
      if ( is_writable(KUUZU::BASE_DIRECTORY . 'Work/Logs') ) {
        ...
        ini_set('error_log', KUUZU::BASE_DIRECTORY . 'Work/Logs/errors.txt');
      }

      if ( in_array('sqlite', PDO::getAvailableDrivers()) && is_writable(KUUZU::BASE_DIRECTORY . 'Work/Database/') ) {
        set_error_handler(array('Kuuzu\\KU\\Core\\ErrorHandler', 'execute'));

        if ( file_exists(KUUZU::BASE_DIRECTORY . 'Work/Logs/errors.txt') ) {
          static::import(KUUZU::BASE_DIRECTORY . 'Work/Logs/errors.txt');
        }
      }
    }
...
// return true to stop further processing of internal php error handler
    ...
    public static function connect() {
      ...
      try {
        static::$_dbh = PDO::initialize(KUUZU::BASE_DIRECTORY . 'Work/Database/errors.sqlite3', null, null, null, null, 'SQLite3');
