...
class PDO
{
    ...
    {
        if (!isset($server)) {
            $server = KUUZU::getConfig('db_server');
        }

        if (!isset($username) && KUUZU::configExists('db_server_username')) {
            $username = KUUZU::getConfig('db_server_username');
        }

        if (!isset($password) && KUUZU::configExists('db_server_password')) {
            $password = KUUZU::getConfig('db_server_password');
        }

        if (!isset($database) && KUUZU::configExists('db_database')) {
            $database = KUUZU::getConfig('db_database');
        }

        if (!isset($port) && KUUZU::configExists('db_server_port')) {
            $port = (int)KUUZU::getConfig('db_server_port');
        }

        if (!isset($driver) && KUUZU::configExists('db_driver')) {
            $driver = KUUZU::getConfig('db_driver');
        }
        ...
        if (!isset($driver_options[\PDO::ATTR_STATEMENT_CLASS])) {
            $driver_options[\PDO::ATTR_STATEMENT_CLASS] = [
                'Kuuzu\\KU\\Core\\PDOStatement'
            ];
        }

        $class = 'Kuuzu\\KU\\Core\\PDO\\' . $driver;
...
// join the query before the comment appeared, with the rest of the dump
                ...
                if ((strtoupper($next) == 'DROP T') || (strtoupper($next) == 'CREATE') || (strtoupper($next) == 'INSERT') || (strtoupper($next) == 'ALTER ') || (strtoupper($next) == 'SET FO')) {
                    ...
                    if (isset($table_prefix)) {
                        if (strtoupper(substr($sql_query, 0, 25)) == 'DROP TABLE IF EXISTS KUU_') {
                            $sql_query = 'DROP TABLE IF EXISTS ' . $table_prefix . substr($sql_query, 25);
                        } elseif (strtoupper(substr($sql_query, 0, 17)) == 'CREATE TABLE KUU_') {
                            $sql_query = 'CREATE TABLE ' . $table_prefix . substr($sql_query, 17);
                        } elseif (strtoupper(substr($sql_query, 0, 16)) == 'INSERT INTO KUU_') {
                            $sql_query = 'INSERT INTO ' . $table_prefix . substr($sql_query, 16);
                        } elseif (strtoupper(substr($sql_query, 0, 12)) == 'CREATE INDEX') {
                            $sql_query = substr($sql_query, 0, stripos($sql_query, ' on kuu_')) . ' on ' . $table_prefix . substr($sql_query, stripos($sql_query, ' on kuu_') + 8);
                        }
                    }
    ...
    public static function getBatchPageLinks(string $batch_keyword = 'page', int $total, string $parameters = '', bool $with_pull_down_menu = true): string
    {
        ...
        if ($number_of_pages > 1) {
            ...
            $string = sprintf(KUUZU::getDef('result_set_current_page'), 1, 1);
        }
    ...
    public static function getBatchPagesPullDownMenu(string $batch_keyword = 'page', int $total, string $parameters = null): string
    {
        ...
        $string = '<form action="' . KUUZU::getLink(null, null) . '" action="get">' . $hidden_parameter .
                  sprintf(KUUZU::getDef('result_set_current_page'), HTML::selectMenu($batch_keyword, $pages_array, $batch_number, 'onchange="this.form.submit();"'), $number_of_pages) .
    ...
    public static function getBatchPreviousPageLink(string $batch_keyword = 'page', string $parameters = null): string
    {
        ...
        $back_string = HTML::icon('nav_back.png', KUUZU::getDef('result_set_previous_page'));
        $back_grey_string = HTML::icon('nav_back_grey.png', KUUZU::getDef('result_set_previous_page'));

        if ($batch_number > 1) {
            $string = HTML::link(KUUZU::getLink(null, null, $parameters . $batch_keyword . '=' . ($batch_number - 1)), $back_string);
        } else {
    ...
    public static function getBatchNextPageLink(string $batch_keyword = 'page', int $total, string $parameters = null): string
    {
        ...
        $forward_string = HTML::icon('nav_forward.png', KUUZU::getDef('result_set_next_page'));
        $forward_grey_string = HTML::icon('nav_forward_grey.png', KUUZU::getDef('result_set_next_page'));

        $string = '&nbsp;';

        if (($batch_number < $number_of_pages) && ($number_of_pages != 1)) {
            $string .= HTML::link(KUUZU::getLink(null, null, $parameters . $batch_keyword . '=' . ($batch_number + 1)), $forward_string);
        } else {
    ...
    {
        if (!isset($this->_table_prefix) && KUUZU::configExists('db_table_prefix')) {
            $this->_table_prefix = KUUZU::getConfig('db_table_prefix');
        }
