 ...
 */
    ...
    public function write($data, $key = null) {
      ...
      if ( !static::hasSafeName($key) ) {
        trigger_error('KUUZU_Cache::write(): Invalid key name (\'' . $key . '\'). Valid characters are a-zA-Z0-9-_');

        return false;
      }

      if ( is_writable(KUUZU::BASE_DIRECTORY . 'Work/Cache/') ) {
        return ( file_put_contents(KUUZU::BASE_DIRECTORY . 'Work/Cache/' . $key . '.cache', serialize($data), LOCK_EX) !== false );
      }
 ...
 */
    ...
    public function read($key, $expire = null) {
      ...
      if ( !static::hasSafeName($key) ) {
        trigger_error('KUUZU_Cache::read(): Invalid key name (\'' . $key . '\'). Valid characters are a-zA-Z0-9-_');
      ...
      $filename = KUUZU::BASE_DIRECTORY . 'Work/Cache/' . $key . '.cache';
 ...
 */
    ...
    public static function clear($key) {
      ...
      if ( !static::hasSafeName($key) ) {
        trigger_error('KUUZU_Cache::clear(): Invalid key name (\'' . $key . '\'). Valid characters are a-zA-Z0-9-_');
      ...
      if ( is_writable(KUUZU::BASE_DIRECTORY . 'Work/Cache/') ) {
        ...
        $d = dir(KUUZU::BASE_DIRECTORY . 'Work/Cache/');

        while ( ($entry = $d->read()) !== false ) {
          if ( (strlen($entry) >= $key_length) && (substr($entry, 0, $key_length) == $key) ) {
            @unlink(KUUZU::BASE_DIRECTORY . 'Work/Cache/' . $entry);
          }
        }
