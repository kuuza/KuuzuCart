 ...
 */
  ...
  class Modules {
    ...
    function __construct($group) {
      ...
      if ( $KUUZU_Cache->read('templates_' . $this->_group . '_layout-' . $KUUZU_Template->getCode() . '-' . KUUZU::getSiteApplication() . '-' . $KUUZU_Template->getPageContentsFilename())) {
        $data = $KUUZU_Cache->getCache();
      } else {
        $data = array();

        $Qspecific = $KUUZU_PDO->prepare('select b2p.boxes_group, b.code from :table_templates_boxes_to_pages b2p, :table_templates_boxes b, :table_templates t where b2p.templates_id = :templates_id and b2p.page_specific = 1 and b2p.content_page in ("*", "' . KUUZU::getSiteApplication() . '/*", "' . KUUZU::getSiteApplication() . '/' . substr($KUUZU_Template->getPageContentsFilename(), 0, strrpos($KUUZU_Template->getPageContentsFilename(), '.')) . '") and b2p.templates_boxes_id = b.id and b.modules_group = :modules_group and b2p.templates_id = t.id order by b2p.boxes_group, b2p.sort_order');
        $Qspecific->bindInt(':templates_id', $KUUZU_Template->getID());
        ...
        if ( count($result) > 0 ) {
          ...
          $Qmodules = $KUUZU_PDO->prepare('select b2p.boxes_group, b2p.content_page, b.code from :table_templates_boxes_to_pages b2p, :table_templates_boxes b, :table_templates t where b2p.templates_id = :templates_id and b2p.content_page in ("*", "' . KUUZU::getSiteApplication() . '/*", "' . KUUZU::getSiteApplication() . '/' . substr($KUUZU_Template->getPageContentsFilename(), 0, strrpos($KUUZU_Template->getPageContentsFilename(), '.')) . '") and b2p.templates_boxes_id = b.id and b.modules_group = :modules_group and b2p.templates_id = t.id order by b2p.boxes_group, b2p.sort_order');
          $Qmodules->bindInt(':templates_id', $KUUZU_Template->getID());
    ...
    function getGroup($group) {
      ...
      if ( isset($this->_modules[$group]) ) {
        foreach ( $this->_modules[$group] as $module ) {
          $class = 'Kuuzu\\KU\\Core\\Site\\Shop\\Module\\' . $this->_group . '\\' . $module . '\\Controller';
    ...
    function isInstalled($code = '', $group = '') {
      ...
      if ( !isset($is_installed) ) {
        $Qcheck = $KUUZU_PDO->query('select id from :table_templates_boxes where code = :code and modules_group = :modules_group');
    ...
    function install() {
      ...
      foreach ( $KUUZU_Language->getAll() as $key => $value ) {
        if ( file_exists(dirname(__FILE__) . '/../languages/' . $key . '/modules/' . $this->_group . '/' . $this->_code . '.xml') ) {
          foreach ( $KUUZU_Language->extractDefinitions($key . '/modules/' . $this->_group . '/' . $this->_code . '.xml') as $def ) {
            $Qcheck = $KUUZU_PDO->prepare('select id from :table_languages_definitions where definition_key = :definition_key and content_group = :content_group and languages_id = :languages_id limit 1');
            ...
            if ( count($result) === 1 ) {
              $Qdef = $KUUZU_PDO->prepare('update :table_languages_definitions set definition_value = :definition_value where definition_key = :definition_key and content_group = :content_group and languages_id = :languages_id');
            } else {
    ...
    function remove() {
      ...
      $Qdel = $KUUZU_PDO->prepare('delete from :table_templates_boxes where code = :code and modules_group = :modules_group');
      $Qdel->bindValue(':code', $this->_code);
      $Qdel->bindValue(':modules_group', $this->_group);
      $Qdel->execute();

      if ( $this->hasKeys() ) {
        $KUUZU_PDO->exec('delete from :table_configuration where configuration_key in ("' . implode('", "', $this->getKeys()) . '")');
      }

      if ( file_exists(dirname(__FILE__) . '/../languages/' . $KUUZU_Language->getCode() . '/modules/' . $this->_group . '/' . $this->_code . '.xml') ) {
        foreach ( $KUUZU_Language->extractDefinitions($KUUZU_Language->getCode() . '/modules/' . $this->_group . '/' . $this->_code . '.xml') as $def ) {
